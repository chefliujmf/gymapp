...
- **⚠️ Button 403 (confirmed 2026-06-25):** `deploy.yml` sets `GH_PROMOTE_TOKEN: ${{ secrets.PROMOTE_TOKEN }}`, but `PROMOTE_TOKEN` is a fine-grained PAT with **Contents+PRs write only — NO Actions: write**. The handler POSTs `actions/workflows/promote-prod.yml/dispatches` (a `workflow_dispatch`), which REQUIRES `actions: write` → GitHub returns **403**. So the in-app button 403s even though the token is present. **Fix:** add **Actions: Read and write** to the PROMOTE_TOKEN fine-grained PAT (GitHub → Settings → Developer settings → Fine-grained tokens → edit → Repository permissions → Actions). The token string is unchanged, so **no redeploy needed** — the button works as soon as the permission is granted. (Workaround meanwhile: `gh workflow run promote-prod.yml` / Actions tab.)
