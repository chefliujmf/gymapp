...
- **Auth = a GitHub App (zero-maintenance, JM directive 2026-06-23 "don't want to maintain it").** `actions/create-github-app-token` mints a fresh **1h** repo-scoped token each run from secrets `APP_ID` + `APP_PRIVATE_KEY`; the App's private key never expires, so nothing to rotate. An **App** token (unlike the built-in `GITHUB_TOKEN`) DOES trigger the PR's `build` check — that's why it's an App, not GITHUB_TOKEN, and why a PAT (which a human must re-mint on expiry) was rejected. App = "platyplus-promote", perms Contents+PRs write, installed on the repo. Repo "Allow auto-merge" ON (`gh api -X PATCH repos/chefliujmf/gymapp -f allow_auto_merge=true`, done). To set the App secrets: `gh secret set APP_ID --repo chefliujmf/gymapp` / `gh secret set APP_PRIVATE_KEY --repo chefliujmf/gymapp < key.pem`.
