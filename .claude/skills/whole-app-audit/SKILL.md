...
0. **SIMULATE with MOCK DATA — do not audit by code-reading alone (JM directive 2026-07-22).** For EACH persona, INVENT a representative profile + benchmarks and create a QA account for it (sex, `info.dob`, `info.goals.focus`+notes, `sports`, `info.mainSport`, `info.trainingDays`, `sportSettings.*` thresholds, repro-state, `coachProfile`) — see [[coach-simulation]] setup. Then **trigger a real plan** (`POST /api/coach/daily-adapt` on QA, or clear the future + re-adapt) and **evaluate the ACTUAL generated plan** (DB + the rendered app), not just the code path. Include the two highest-harm edge personas EXPLICITLY: a **teen with NO dob entered** and a **beginner with an EMPTY goal + no benchmarks** — these expose fail-unsafe defaults that code-reading rationalizes away. Static code review is the SECOND layer; the runtime simulation is the first. Clean up throwaway accounts after (keep a stable set for re-use).
1. **Environment: PROD, real Chrome** (skill `platyplus-browser-testing`; JM signed in on QA+prod). Prod = what users actually get. Use QA for the mock-data simulations (read-only toward intervals, safe to spin personas + re-adapt).
