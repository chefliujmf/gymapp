...
**Companion rule — MAINTAIN the in-app backlog too (#438/#440, JM 2026-07-08):** the backlog is ALSO in the app (Admin → Backlog), backed by the SHARED global store `app_meta.backlog` (`store.backlog = {triage, added}`). **Every session, read JM's live triage** (`GET /auth/admin/backlog`, or `select doc->'backlog' from app_meta where id=1`) alongside `FEEDBACK-LOG.md` and let it steer: his **status** OVERRIDES the .md-derived status (incl. `review` = a fresh user report to triage, Done = his ✅), his **priority** OVERRIDES queue order, skip **discarded**, and **FOLD** his comments + app-added/reported items into the numbered `.md` entry (the .md stays authoritative; the app list is generated from it by `scripts/build-backlog.mjs`). Keep both the .md AND the item's app status current as you work. See memory `platyplus-admin-backlog`.

**Companion rule — DRIVE the full status CYCLE, both layers (JM 2026-07-08):** statuses are a lifecycle, not labels — MOVE items through it as you work, updating BOTH the `.md` emoji AND the in-app store status (JM's in-app status wins, so an unchanged app status looks stale). The cycle + who owns each hop:
`⬜ To do` →(**you** start it)→ `🔨 Building` →(**you** ship to QA)→ `🧪 To test` →(**JM** tests)→ `✅ Tested✓`→(promote)→`Done`, **or** `Tested✗ fail` →(**you** pick it back up)→ **`🔨 Building`** →… loop. `Under review`(user report) →(triage)→ `To do`; `Discarded` anytime.
Concretely: **when you start reworking a FAILED item, set it back to Building** (`.md` 🔨 + `PUT /auth/admin/backlog/:n {status:'build'}` or the store) so JM sees it's in progress again; when you re-ship it, move it to To test. Never leave a failure sitting at `fail` while you're actively fixing it. (Ties #447 — this cycle IS the clarified model.)
