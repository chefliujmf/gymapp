...
// #403 — the synthesis: FTP·TTE·CP·W′·EF → athlete type + coach reads + a no-test training focus.
describe('athleteProfile', () => {
  it('JM: high FTP + short TTE → punchy threshold, focus on extending TTE (no test)', () => {
    ...
    expect(p.focus.some((f) => /no formal test/i.test(f))).toBe(true)
