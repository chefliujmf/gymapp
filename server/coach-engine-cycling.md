...
- ALWAYS verify the week's TOTAL before finalising: sum the planned sessions' TSS (each session's load is shown on the calendar) and confirm it lands in the band above (≈ CTL×7 flat to CTL×8 build; ~250-320 here). A week at roughly double the band — e.g. ~440 TSS at CTL ~32, projecting Form past -20 — is a planning error, not a build; cut it back. Never place two long-or-hard days back-to-back; alternate load and easy.
