...
const REVIEW_OWN_TERMS = 'FRAME the session on its OWN terms: only mention an adjacent session ("the day after X", "coming off Y", yesterday/tomorrow) when you have VERIFIED it in get_recent_activities or the calendar — NEVER assume surrounding-day context (no "day after gym" unless a gym session is actually on record). #699 (JM) — do NOT write a "what\'s next" / next-steps line at all: LEAVE the `next` field EMPTY. The review reflects THIS session + the athlete\'s progress only; the plan lives elsewhere. #698 (JM) — NEVER mention "pump" (or "no pump"/"zero pump") — that concept was removed and is not relevant.'
...
function triggerActivityReview(user, id, fb, icuActivityId) {
  ...
  const planLine = plan
    ? ` This session was meant to fulfil the PLANNED workout "${plan.title}"${plan.objective ? ` — objective: ${plan.objective}` : ''}. You MUST open your review by comparing what they ACTUALLY did to THAT planned session (and ONLY that day's plan — never reference or invent another day's workout): (a) if they hit the plan's intent + intensity, say so and score it well; (b) if they did something DIFFERENT — e.g. an easy ride when a threshold/quality session was planned, or shorter/longer/harder than prescribed — NAME the gap plainly ("this was an easy Z2, not the planned threshold work"), do NOT praise it as if it matched the plan, and let the SCORE reflect the miss (a good easy ride that skipped the planned quality work is NOT a high score against that plan). A deliberate, sensible swap can still be acknowledged as the right call, but the review must state the deviation either way.`
...
Use create_swim / create_ride / create_run / create_workout, each to its own zones, balanced to total load and periodized to the A-race.`
  ...
  const restDays = (user.restDates || []).filter((d) => d >= todayIso).slice(0, 21)
  ...
  if (gymMainSport) {
    ...
    tail += `\n\n# THIS WEEK — YOUR SPORT IS THE GYM. This athlete's ENTIRE training week is the **# THIS WEEK'S GYM SESSIONS** block above. **⚠️ THE PLAN IS BUILT WITH \`create_workout\` — NOTHING ELSE. Your FIRST and MAIN job this pass: ToolSearch for \`create_workout\` and CALL it ONCE PER SESSION below to actually schedule each gym day.** \`set_gym_target\` only records a stats band (sets/muscle) — it creates ZERO workouts; **if you set a target but do not call \`create_workout\`, the athlete is left with an EMPTY plan — that is a FAILED adapt.** So: build the ${Number(user.info?.trainingDays) || 'planned'} gym sessions with create_workout FIRST (that IS the whole plan), and only AFTER they exist may you optionally set_gym_target. Their dose (rep scheme, %1RM, volume) follows their GOAL per # GYM FOCUS — a MUSCLE goal → hypertrophy (6-12 reps, higher volume), a STRENGTH / 1-RM goal → heavy low-rep strength (3-6 reps, >85% 1RM, low volume, long rests), general → moderate; do NOT default to hypertrophy for everyone. There is NO endurance quality-day count, intensity ceiling, or ride/run/swim here — do NOT invent cardio. Fill the ${DAILY_HORIZON}-day horizon with their gym days, spacing hard sessions with recovery/rest between.`
  } else {
  ...
  else if (shape.impact === 'walk_run') tail += `\n\n# POSTPARTUM — IMPACT: reintroduce impact GRADUALLY. WALK-RUN intervals only (e.g. run 1 min / walk 2 min, building the run portion as tolerated) — NOT continuous running yet. Keep it easy; stop at any leaking / heaviness / pressure and defer to her pelvic-floor PT.`
