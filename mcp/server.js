...
server.tool('set_activity_text',
  'Set the PUBLIC title + description on a COMPLETED intervals activity (this SYNCS TO STRAVA and is visible to others). PUBLIC-SAFE ONLY: no score, health/pain/niggles, fatigue/recovery status, feelings ("felt good so I…"), or future-plan protection (those go in save_coach_review, the private Notes thread). TWO DIFFERENT VOICES: the TITLE is HUMAN + creative, a normal athlete Strava title (route, session type, a segment/KOM/Local Legend, keep it fun, e.g. "KOM on the Backroads" or "Another Local Legend, Relaxed Miles"); the DESCRIPTION is SCIENTIFIC, name the training zones (Z1 to Z5) and the ADAPTATION (aerobic base, threshold, W prime, VO2max) with the numbers (avg power, IF, TSS), never vibe words ("easy", "hard", "relaxing spin", "classic spin", "proof you dont have to go hard"). NEVER use an em-dash in either field, use commas or periods. Follow instructions_public_text.',
