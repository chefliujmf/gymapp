...
server.tool('search_exercises',
  'Search the Platyplus exercise library by name; returns ids (exId), the equipment tag, and demo media. Use the exId in create_workout. Every result is a COMPLETE exercise with a real VIDEO demo (image-only entries are never returned) — so always pick from these and never invent an exId. Results are ranked best-match first, so the TOP hit is usually the right movement. **SPEED — ALWAYS BATCH: pass `queries` = the ARRAY of EVERY move you need for the whole session (warm-up + main + cool-down) in ONE call** — it returns a { name: results } map. Never call this once per exercise (that is 20-30 slow round-trips; batch them all into a single call). IMPORTANT: pass `equipment` = the athlete\'s OWNED gear (see the profile) so you only ever prescribe exercises they can actually do; "Bodyweight" needs nothing.',
  { queries: z.array(z.string()).optional().describe('ARRAY of name fragments to look up AT ONCE — PREFER this: one call for the whole session, e.g. ["goblet squat","romanian deadlift","dumbbell row","cat cow"]'), query: z.string().optional().describe('single name fragment (only if you truly need one; otherwise use queries[])'), equipment: z.string().optional().describe('comma-separated owned equipment to limit to, e.g. "Dumbbell,Bodyweight,Bands"'), limit: z.number().int().min(1).max(100).optional() },
  wrap((a) => {
    const eqp = encodeURIComponent(a.equipment || '')
    if (Array.isArray(a.queries) && a.queries.length) return api('GET', `/api/exercises?qs=${encodeURIComponent(a.queries.join('|'))}&equipment=${eqp}&limit=${a.limit || 6}`)
    return api('GET', `/api/exercises?q=${encodeURIComponent(a.query || '')}&equipment=${eqp}&limit=${a.limit || 20}`)
  }))
...
server.tool('get_metrics',
  "Read the athlete's computed performance PROFILE (live from intervals.icu, READ-ONLY). Per sport — cycling: FTP · eFTP · Critical Power (CP) · W' (anaerobic reserve, kJ) · TTE (time-to-exhaustion at FTP, s) · Efficiency Factor; running: threshold pace · Critical Speed (CS) · D' (m) · TTE · EF; swimming: CSS (critical swim speed) · D' (m) · swim TTE · SWOLF (efficiency) — plus a synthesised athlete TYPE with a training focus, per sport. ALL model-derived from their power/pace curve — NO exhaustion or lab test needed; normal hard efforts sharpen it. Use this to coach BEYOND one threshold number: read the PROFILE — short TTE (≪30 min) ⇒ extensive threshold work (grow how long they hold threshold) and/or ease the threshold toward the modelled value; small W'/D' ⇒ short near-max repeats; rising EF ⇒ the aerobic base is working, keep it. SWIM specifically: size threshold SETS off their swim TTE, sprint reps off D', and drive technique/drill volume off SWOLF (rising SWOLF ⇒ more efficiency work — it's the swim analogue of EF). Returns { connected:false } if intervals isn't connected. Theory: docs/beyond-ftp-metrics.md + docs/tte.md.",
  {},
...
server.tool('get_recent_activities',
  "Read the athlete's recently COMPLETED activities from intervals.icu: each has `id` (the activity id — pass it to save_coach_review / set_activity_text to review or annotate THAT activity), `date` (the athlete's LOCAL date), `when` + `daysAgo` (an AUTHORITATIVE relative-day label computed in the athlete's timezone, e.g. `when:\"yesterday\"` / `\"2 days ago\"` / `\"today\"` — ALWAYS use this when you mention when a session happened; NEVER eyeball 'yesterday' from the raw date, you'll be a day off), type, indoor/outdoor, duration, distance, avg HR, avg power, Load (TSS), intensity (IF), RPE, feel, `reviewed` (true once you've reviewed it — the reviewed/NOT-reviewed tracker; `reviewed:false` means it's still waiting for your review), and `awaitingFeedback` (true = the athlete has NOT logged how it went → NEVER review it: a review REQUIRES their feedback, there is no data-only review, and the server will 409 you if you try. Only review a session that has `reviewed:false` AND `awaitingFeedback:false`. Their feedback submit fires the review; the flag clears once they log it). READ-ONLY; returns { connected:false } if intervals.icu isn't connected.",
  { days: z.number().int().min(1).max(60).optional().describe('lookback days; default 14') },
...
const COACHING = {
  ...
  // #765 — PIN a session the ATHLETE explicitly asked to place/move. Set `pinned:true` on EVERY session you create or
  // move to honour an explicit athlete reschedule request (e.g. "move today's session to tomorrow and do an easy ride
  // today" → pin BOTH the easy ride you put today AND the session you moved to tomorrow). A pinned session is STICKY:
  // the automated daily-adapt / horizon-fill CANNOT move, remove, or rest-day over it (the server 409s the attempt), so
  // the athlete's change can't be silently reverted. Do NOT pin sessions YOU chose in a normal adapt — only ones the
  // athlete specifically requested. To change a pinned session later at the athlete's explicit request, first re-call
  // with `pinned:false` (release it), then move/remove it.
  pinned: z.boolean().optional().describe('#765 — true = the athlete explicitly asked for THIS session on THIS day; makes it sticky so the daily-adapt cannot revert it. Only set on an explicit athlete request; false to release.'),
}
const coachingOf = (a) => ({ objective: a.objective, cues: a.cues, tip: a.tip, success: a.success, recovery: a.recovery, fuel: a.fuel, mind: a.mind, ...(a.pinned != null ? { pinned: a.pinned } : {}) }) // #765 — forward the athlete-pin flag
server.tool('create_workout',
  'Schedule a strength/gym workout on a date. Platyplus is the MASTER and mirrors to intervals.icu. Re-call with the same id to UPDATE. THREE HARD REQUIREMENTS (the tool REJECTS the plan otherwise, so build them in up front): (1) a WARM-UP and a COOL-DOWN, each broken into INDIVIDUAL moves — one entry per move, tagged section:"warmup"/"cooldown", every one with a real library exId from search_exercises (arm circles, leg swings, high knees, cat-cow, jogging in place, glute bridge, targeted stretches…); NEVER cram moves into one "Warm-up: A, B, C" line. VARIETY (#573): FIRST read the athlete\'s last gym session with list_schedule (plans carry the full exercises) so you can step OFF it — do NOT reuse the last warm-up, and ROTATE the movement-prep flow every session (mobility / dynamic / activation / movement-ramp / KB-prep), and ROTATE accessories within their pattern (horizontal pull = barbell row → chest-supported row → 1-arm DB row → cable row → inverted row; NOT "rowing" every time). KEEP the 1–3 MAIN compound lifts stable week to week so load/reps progress. Every move comes from search_exercises (video-only pool), so each has a demo — look them ALL up in ONE batched search_exercises(queries=[…]) call, never one at a time. See coach-engine-strength "VARIETY". #636 — MUSCLE-GROUP BALANCE: across the week cover every pattern; ARMS (biceps+triceps) + a carry are REQUIRED, never skipped — follow the injected runtime GYM BALANCE block (stable mains for progress, fresh rotated accessories, full coverage). #648 — REPS + LOAD by FOCUS: take the PRIMARY compound-lift reps, %1RM and tempo from that block\'s REP SCHEME — an endurance-support athlete (cyclist/runner) trains the MAINS HEAVY + LOW-rep (~3-6) with a fast/explosive concentric, NOT to failure (force + economy, minimal mass); a strength goal ~3-5 heavy; only a hypertrophy/muscle goal uses 6-12. Accessories/arms may run moderate. Do NOT default legs to 3×10. (2) Order the MAIN set by equipment (barbell → dumbbell → cable/machine → bodyweight/trunk) so the athlete isn\'t ping-ponging stations. (3) Every single-side move (Pallof press, split squat/Bulgarian, single-arm/leg, side plank, suitcase carry, Copenhagen…) MUST be prescribed both sides — set eachSide:true (renders "each side") or write explicit left/right entries. Optionally attach the coaching shell — objective + success criteria + a short fuelling cue for THIS session ONLY (you author every cue, keep them consistent + non-contradictory); do NOT set recovery or mind (removed from the workout view). ONE SESSION/DAY: the app REJECTS (409) a 2nd session on a day already at the athlete\'s max, AND any session that would exceed their HARD weekly training-days cap (move or combine, do NOT add a new day past the cap) — re-call with that day\'s existing id to fold moves in, or move it to a free day. Gym EXERCISES live + render in Platyplus (intervals has no gym model — it only gets a deep-link) — ALWAYS send the full structured exercises here; to UPDATE a gym session, re-call with its SAME id (never leave a gym session with no exercises).',
  {
    date: DATE,
    title: z.string(),
    ...
    exercises: z.array(z.object({
      ...
      mode: z.enum(['reps', 'timed']).optional().describe("#664 — COPY from the exId's `timed` flag in search_exercises: a move that comes back timed:true (plank, stretch, mobility, carry, dynamic warm-up) is 'timed' (set `seconds`), everything else is 'reps'. Do NOT send a plank/stretch as reps."),
      sets: z.number().int().optional(),
      reps: z.number().int().optional().describe('take PRIMARY compound-lift reps from the injected GYM BALANCE rep scheme for the athlete\'s focus (endurance-support ~3-6 heavy · strength ~3-5 · hypertrophy 6-12); accessories/arms may run moderate. Do NOT default legs to 10.'),
      ...
      eachSide: z.boolean().optional().describe('#664 — COPY from the exId\'s `eachSide` flag in search_exercises (single-arm/leg, split squat, lunge, Pallof, side plank…). The dose is PER SIDE, renders "each side" (L + R). REQUIRED on unilateral moves or the plan is rejected.'),
      tempo: z.string().optional().describe('lifting TEMPO as 4 digits eccentric-pauseBottom-concentric-pauseTop, e.g. "3-1-1-0" = 3s lower, 1s pause, 1s lift, 0s top. Use the injected REP SCHEME tempo: a FAST/explosive concentric like "3-0-1-0" for endurance-support / strength / power MAINS (fast bar-speed builds force with little mass), a slower eccentric "3-1-1-0" only for a hypertrophy/muscle goal. Omit for pure mobility/holds.'),
...
const makeEndurance = (sport) => wrap((a) => api('POST', '/api/plan', {
  ...
  ...(a.indoor != null ? { indoor: a.indoor } : {}), // #479 indoor(ERG specific) vs outdoor(range)
  ...coachingOf(a),
}))

server.tool('create_ride',
  'Schedule a structured bike workout (power intervals). Platyplus is master; mirrors to intervals.icu as a real workout (steps → head unit/trainer). Re-call with same id to update. ONE SESSION/DAY: the app REJECTS (409) a 2nd session on a day already at the athlete\'s max, AND any session that would exceed their HARD weekly training-days cap (move or combine, do NOT add a new day past the cap) — never book two short rides; COMBINE into that day\'s existing session (re-call with its id) or move it to a free day. The app auto-computes the planned LOAD (TSS) from your segments and sends it to intervals so Form/CTL/ATL project correctly — you do NOT set load. TARGETS: always give the CENTER %FTP per steady segment (powerStart==powerEnd). INDOOR vs OUTDOOR (#479 / #728): OUTDOOR-FIRST — DEFAULT to outdoor (omit/`false`) → the app auto-widens each steady target into a rideable min–max band (wide endurance, tight threshold). Set `indoor:true` (TRAINER/ERG holds the SPECIFIC watt) ONLY when the weather genuinely blocks outside riding (thunderstorm, heavy/freezing rain, ice, dangerous heat/cold, unsafe wind, darkness) OR the athlete prefers/accepts the trainer. A quality/threshold/VO2 session is NOT a reason to go indoor, and ERG "cleaner power" NEVER overrides outdoor-first — the athlete can flip it on the session. Either way NEVER write a range into the label. (Warm-ups stay a ramp, cool-downs stay easy.) Optionally attach the coaching shell — objective + success criteria + a short fuelling cue for THIS session ONLY (you author every cue, keep them consistent + non-contradictory); do NOT set recovery or mind (removed from the workout view).',
  { date: DATE, title: z.string(), ftp: z.number().optional().describe('override FTP in watts'), indoor: z.boolean().optional().describe('#479 — true = INDOOR/ERG (a smart trainer holds the specific watts); false/omit = OUTDOOR (steady targets auto-widen to a rideable power range). The athlete can flip this per session.'), segments: SEGMENTS, notes: z.string().optional(), ...COACHING, id: z.string().optional() },
  makeEndurance('ride'))

server.tool('create_run',
  'Schedule a structured run. Coach it the RUNNING way — Daniels E/M/T/I/R off threshold PACE (see SEGMENTS for the zones), not by bike power. The app converts each segment % to the athlete\'s real min/km. Re-call with same id to update. ONE SESSION/DAY: the app REJECTS (409) a 2nd session on a day already at the athlete\'s max, AND any session that would exceed their HARD weekly training-days cap (move or combine, do NOT add a new day past the cap) — COMBINE into that day\'s existing session (re-call with its id) or move it. The app auto-computes the planned LOAD (TSS) and sends it to intervals so Form projects — you do NOT set load. Optionally attach the coaching shell — objective + success criteria + a short fuelling cue for THIS session ONLY; do NOT set recovery or mind (removed from the workout view).',
...
server.tool('create_swim',
  'Schedule a SWIM workout (distance sets on a send-off, prescribed by CSS zone). Platyplus is master; mirrors to intervals.icu. Re-call with same id to update. ONE SESSION/DAY + respect the HARD weekly training-days cap — combine/move, do NOT add past the cap. Build it the swim way: warm-up → DRILLS (technique EVERY session — catch-up, fingertip-drag, sculling, kick-on-side) → MAIN set → cool-down. Coach by CSS zone (3 = CSS/threshold). Follow the injected WEEK SHAPE block for HOW MANY hard sets this week — do NOT fix a weekly count here. The app computes total distance, duration, and sTSS load. Distances in metres (yard pool reads the same). Optionally attach the coaching shell — objective + success criteria + a short fuelling cue for THIS session ONLY; do NOT set recovery or mind (removed from the workout view).',
  {
...
// and recovery ITEMS are parked (roadmap). #623 (JM): the planned-workout DESCRIPTION carries ONLY objective +
// success criteria + a fuelling cue for THAT session — NO recovery, NO Eat/meal/supplement, NO mind, and NO past/
// future session. The coach plans TRAINING only; every cue is coach-authored + must stay consistent (no contradiction).
...
server.tool('set_rest_day',
  'Mark a day as a DELIBERATE REST day — use this WHENEVER the athlete asks to rest a specific day (or asks to move a session off a day with nothing replacing it). It REMOVES any session on that day and makes the rest STICKY: no later daily-adapt or horizon-fill will re-add a session there (the server 409s any attempt), and it does NOT count toward the weekly training-days target. This is how a rest the athlete asked for is actually respected instead of getting silently re-filled. Reversible with clear_rest_day.',
  { date: DATE },
  wrap((a) => api('POST', '/api/rest-day', { date: a.date, rest: true })))
server.tool('clear_rest_day',
  'Remove the deliberate REST marker from a day (the athlete changed their mind and wants to train it), so you can schedule a session there again.',
  { date: DATE },
  wrap((a) => api('POST', '/api/rest-day', { date: a.date, rest: false })))
server.tool('save_coach_review',
  'Save YOUR PRIVATE review of a COMPLETED workout. ⚠️ ONLY call this for a session the athlete has ALREADY given feedback on (get_recent_activities shows `awaitingFeedback:false`) — a review REQUIRES their feedback and the server 409s a review for a session with none (there is NO data-only review). Shows in the athlete\'s post-workout view + Progress, AND (when activityId is given) is auto-posted to the intervals Notes/comment thread in the standard "Coach note" format. This is the PRIVATE channel — put score, mind, recovery, health, and next-plan context HERE, never in the public title/description (use set_activity_text for that). Give a one-line verdict, 2-4 takeaways, and what\'s next.',
  { date: z.string().describe('the workout date, YYYY-MM-DD'), verdict: z.string().optional().describe('one-line overall verdict, e.g. "Solid threshold work — held the watts, legs faded late."'), takeaways: z.array(z.string()).optional().describe('2-4 short bullets shown on the Progress card'), execution: z.array(z.string()).optional().describe('what went well / the main limiter'), body: z.string().optional().describe('body-maintenance action or "no new issue"'), next: z.string().optional().describe('the OBJECTIVE this session moves toward — how it builds their goal/limiter. Talk objectives + progress, NOT the next workouts: never name/list upcoming scheduled sessions ("Today… Tomorrow…" is wrong) and no recovery/sleep here.'), recovery: z.string().optional().describe('recovery + nutrition/supplement note (posted as a second Recovery/Supplements comment)'), sport: z.string().optional().describe('ride | run | gym'), score: z.number().optional().describe('execution score 0-10 (shown as N/10)'), planId: z.string().optional().describe('the planned workout id, if reviewing one'), activityId: z.string().optional().describe('the intervals activity id, if reviewing a completed device activity — REQUIRED for the note to sync to intervals') },
