...
server.tool('set_activity_text',
  'Set the PUBLIC title + description on a COMPLETED intervals activity (this SYNCS TO STRAVA and is visible to others). PUBLIC-SAFE ONLY: describe the workout itself, its type, route/place, terrain, ZONES, structure, conditions, duration. NEVER include score, health/pain/niggles, fatigue/recovery status, feelings ("felt good so I…"), or future-plan protection, those go in save_coach_review (the private Notes thread). VOICE = SCIENTIFIC, not cocky: name the training zones (Z1 to Z5) and the ADAPTATION (aerobic base, threshold, W prime, VO2max), never vibe words ("easy", "hard", "relaxing spin", "classic spin", "snagged a Local Legend"). NEVER use an em-dash, use commas or periods. e.g. title "Threshold 3x10 at FTP" or "Z2 Aerobic Base, Rolling Backroads"; the description names the zones + the adaptation, factual not a brag. Follow instructions_public_text.',
