...
server.tool('create_ride',
  'Schedule a structured bike workout (power intervals). Platyplus is master; mirrors to intervals.icu as a real workout (steps → head unit/trainer). Re-call with same id to update. ONE SESSION/DAY: the app REJECTS (409) a 2nd session on a day already at the athlete\'s max, AND any session that would exceed their HARD weekly training-days cap (move or combine, do NOT add a new day past the cap) — never book two short rides; COMBINE into that day\'s existing session (re-call with its id) or move it to a free day. The app auto-computes the planned LOAD (TSS) from your segments and sends it to intervals so Form/CTL/ATL project correctly — you do NOT set load. TARGETS: always give the CENTER %FTP per steady segment (powerStart==powerEnd). INDOOR vs OUTDOOR (#479): set `indoor:true` for a TRAINER/ERG session → the app keeps the SPECIFIC watt (ERG holds it exactly); omit/`false` for OUTDOOR → the app auto-widens each steady target into a rideable min–max band (wide endurance, tight threshold). Infer it (trainer intervals / bad-weather / short quality → indoor; long endurance / good weather → outdoor) — the athlete can flip it on the session. Either way NEVER write a range into the label. (Warm-ups stay a ramp, cool-downs stay easy.) Optionally attach the coaching shell — objective + success criteria + a short fuelling cue for THIS session ONLY (you author every cue, keep them consistent + non-contradictory); do NOT set recovery or mind (removed from the workout view).',
