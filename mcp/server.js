...
server.tool('get_recent_activities',
  "Read the athlete's recently COMPLETED activities from intervals.icu: each has `id` (the activity id — pass it to save_coach_review / set_activity_text to review or annotate THAT activity), `date` (the athlete's LOCAL date), `when` + `daysAgo` (an AUTHORITATIVE relative-day label computed in the athlete's timezone, e.g. `when:\"yesterday\"` / `\"2 days ago\"` / `\"today\"` — ALWAYS use this when you mention when a session happened; NEVER eyeball 'yesterday' from the raw date, you'll be a day off), type, indoor/outdoor, duration, distance, avg HR, avg power, Load (TSS), intensity (IF), RPE, feel, `reviewed` (true once you've reviewed it — the reviewed/NOT-reviewed tracker; `reviewed:false` means it's still waiting for your review), and `awaitingFeedback` (true = fresh session, today/yesterday, the athlete hasn't logged how it went yet → in the daily review pass, SKIP it: reviewing now beats their input and causes a double-review when they submit; wait for their feedback or the next pass). READ-ONLY; returns { connected:false } if intervals.icu isn't connected.",
  { days: z.number().int().min(1).max(60).optional().describe('lookback days; default 14') },
