...
609. 🧪 **PREGNANCY plan was UNSAFE — coach programmed THRESHOLD + SWEET-SPOT intervals for a pregnant athlete (JM 2026-07-20).** After the #608 timeout fix let Xenia's adapt finish, the plan it built had "Threshold Run 3×3:30" + "Sweet Spot Run" — CONTRAINDICATED in pregnancy (protocol = MAINTAIN only, easy/endurance by RPE + talk test, never quality/threshold/VO2/max). ROOT CAUSE: the computed `generatePlanSkeleton` is NOT pregnancy-aware — it hands the coach normal periodized QUALITY days (threshold/sweetspot/vo2 rotation), and the coach was told "APPLY the computed plan, don't re-derive" so it applied the hard sessions (the #PREGNANCY prompt block got outranked by the skeleton). FIX (`plan-skeleton.js` + `buildSkeletonForUser`): the skeleton is now pregnancy-aware — every `quality` day is demoted to `easy` (→ endurance zone) and load is capped (≤45 TSS/day maintenance ceiling) when `pregnant`. Unit-tested (`src/plan-skeleton.test.ts`: pregnant → 0 threshold/sweetspot/vo2 days; control non-pregnant still gets quality). ⚠️ Her CURRENT plan still has the bad sessions — needs a REGEN after this deploys. **Route:** plan skeleton (pregnancy). ✅ fixed + tested (dev); deploy + regen pending.
