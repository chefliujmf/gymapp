...
765. ⬜ **[JM 2026-07-23·HIGH·bug] "Move today's session to tomorrow + do an easy ride today" was NOT executed.** JM asked the coach (chat) to move today's (07-23) "Threshold 4×10" ride to tomorrow and swap in an easy ride today. VERIFIED in prod: the threshold plan `mcp-r2jlyb4m` is STILL on 07-23 (updatedAt=today, so it was touched but not moved), 07-24 is EMPTY, and NO easy-ride plan was created for today. JM then did the easy ride outdoors (`i168584107` "St-Lambert Road Cycling"), which is orphaned — unlinked to any plan (feeds #766). So a natural-language reschedule-and-substitute request silently didn't change the plan. Route: the chat coach's move/create tools (MCP `create_ride`/move, upsertPlan) + whether a conversational reschedule reliably (a) moves the named session to the target day and (b) lays down the substitute session on today. Enforce/verify the plan actually changed; the coach must not just reply "done" without the plan reflecting it.
