...
715. ⬜ **[audit#1 CRITICAL] Swim shape-clamp is a NO-OP for real swim plans (create_swim emits no zone segments; enforceShape clamps nothing; green test hides it).** Emit per-set zone→%CSS as a segment-equivalent + clamp at save under intensityCeiling; rewrite the test to feed real create_swim output. Route: icu-steps.js/mcp create_swim + shape-enforce.js + server.js.
