...
763. ⬜ **[audit-sim·high] A GYM-focused persona's daily-adapt HANGS (no completion, 0 plans).** The whole-app sim: sim-bodybuilder (gym main sport, hypertrophy) triggered a daily-adapt that logged only the "[chat] req" line — no completion, no tool calls, 0 plans after 5+ min (jmfiset/xenia complete in ~63s). The gym build path (create_workout → many search_exercises to resolve exercise names + assembled sessions) appears to stall for a fresh no-history gym persona. Route: runCoachTask/MCP search_exercises/create_workout for a gym-only fresh user — add a timeout + investigate why the gym adapt doesn't complete. (Blocks full-persona content grading + is a real risk for gym users' adapts.)
