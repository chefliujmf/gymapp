...
510. 🗺️ **Metric duration/season CURVES (power + pace) — parked for the roadmap (JM 2026-07-13: "not good enough yet").** After a long iteration, the season-compare curve isn't up to standard: cycling seasons fit IDENTICAL CP/W′ (overlapping model lines), running pace is GPS-glitch-riddled (raw 0:04/km; the all-time CS fit returns an absurd D′=740 m), and our TRAILING season windows make this≈last-season. REMOVED `<SeasonCompare>` from CyclingStats + RunningStats (curve + best-efforts table). KEPT the benchmark CARDS (FTP/CP/W′/TTE/VO₂max/threshold/CS/D′/MaxHR) + the trend charts (EF/eFTP/pace) — those are fine. Components (`SeasonCompare`, `PowerCurveChart`/`PaceCurveChart`/`DurationCurve`) stay in the codebase. **REVISIT** with true intervals-parity: calendar-year seasons (#415), gradient-adjusted pace, robust outlier rejection / monotonic-hull, and the CS/CP model overlay. Research + cap logic saved in memory `platyplus-chart-standard`. **Route:** stats.
