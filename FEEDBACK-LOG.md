...
521. 🧪 **Profile page reorg — group "What your coach knows about you" (JM 2026-07-15, mock Option B).** The coach-facing bits were scattered: goal SELECTIONS (chips) in one block, the free-text NARRATIVE (`coachProfile`) on a separate `/profile/athlete` page, the LEARNING toggle in a third spot. Merged into ONE card at the top of Profile (`src/CoachKnowledge.tsx`): **✍️ You told it** = goal chips + a collapsible **"Your coach notes" (Edit ›)** = `coachProfile` (where the #519 migration landed the goal + cottage); **📈 It's learning** = the learn-from-check-ins toggle + a **"See your full stats ›"** link. Benchmarks STAY on Stats (not moved, per JM). Rest of Profile grouped under **⚙️ Setup & about you**. Removed the separate GoalsPicker section, the athlete-profile link, and the old "Your data" section. Inline styles only (no `styles.css` collision with the concurrent UX chat). **Note:** the old goals "in your words" box (`info.goals.notes`) edit surface was dropped in favor of the richer `coachProfile` narrative — existing notes stay in the DB + are still read by the coach. Mock: `mockups/profile-coach-knowledge.html`. **Test (QA):** Profile shows ONE "What your coach knows about you" card at the top; tap "Your coach notes" → edit → Save persists; chips + learn toggle save; Stats reachable via the link; setup sections still save. **Route:** UI (Profile).
