...
97. 🔨 **Ride player: power shows TARGET (130W) not live trainer watts; HR = 0** — connected on phone, started a ride; power reads 130W (= 50% FTP target) while not pedaling, and HR stays 0. So live trainer power (FTMS Indoor Bike Data / CPS) and HR notifications aren't flowing into the displayed values (or the UI shows target instead of actual). Investigate RidePlayer data binding (ble.live.power / ble.bpm) + the GATT notification parsing/role detection.
98. ✅ **(expected) BT pairing is per-browser, not synced** — JM paired on phone, expected it on desktop too. Web Bluetooth grants live in the BROWSER that paired (the app/account can't sync them); the phone auto-reconnects there, desktop is a separate empty list. Not a bug — platform constraint. (Desktop Mac Chrome also couldn't see the sensors at all earlier.) Explained.
99. 🔨 **macOS Chrome can't discover HR/trainer — needs a real fix (JM: "not acceptable")** — phone BT off, sensors broadcasting, macOS Chrome scans empty. STEP 1 (free): verify System Settings → Privacy & Security → Bluetooth → Chrome = ON + restart Chrome (the #1 "scans nothing" cause on modern macOS). STEP 2 if still dead: Web Bluetooth on macOS Chrome is genuinely weak → build a **local sensor bridge** (#100). Works fine on Android Chrome (phone).
101. ⬜ **Bridge isn't distributable to non-technical users (JM caught this)** — the run-a-node-script bridge (#100) is fine for JM/technical users but NOT for end users on their own Macs (npm install + native build + Terminal Bluetooth grant = non-starter). DEVICE-SUPPORT REALITY: phone Android Chrome = native Web BT (the official sensor path for everyone); Windows Chrome = native; macOS Chrome/Firefox/Safari/iPhone = no Web BT → bridge (power users) or phone. For mass non-technical macOS-desktop sensors the real answer is a SIGNED packaged companion app (one-click .dmg menubar, bundles the bridge) — a real project (Apple dev signing + notarization), do ONLY if that demand appears. For now: keep the bridge as an optional power-user tool; PHONE is the official sensor path. (Aligns with the NON-TECHNICAL-USER design rule.)
100. 🔨 **Local sensor bridge (BLE → WebSocket) — BUILT (power-user tool, see #101)** — `tools/sensor-bridge` (noble + ws): reads HR/FTMS/CPS over native Bluetooth + streams to the ride player over `ws://localhost:8124` (+ ERG control point write). Client (BleContext) auto-connects the bridge and pipes into the SAME live/bpm pipeline; RidePlayer live row + device panel reflect it. BONUS: because it's a WebSocket (not Web Bluetooth), it works in **Firefox/Safari/any browser**, not just Chrome/Edge. Run: `cd tools/sensor-bridge && npm i && npm start`. TODO: JM runs `npm i` once (native build + grant Terminal Bluetooth) and tests.
