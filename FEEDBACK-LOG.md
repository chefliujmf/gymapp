...
94. ⬜ **Bluetooth "add a device" found neither HR nor trainer (DEV)** — the add-device (Web Bluetooth) flow didn't discover the HR strap or the smart trainer. Investigate: Web Bluetooth needs HTTPS (localhost ok) + correct GATT service filters (heart_rate 0x180D, cycling_power 0x1818, FTMS 0x1826); check the device filters + that the browser/OS BT is reachable in DEV.
