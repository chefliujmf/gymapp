...
713. ⬜ **[audit#5 SAFETY] Postpartum clamps INTENSITY but not IMPACT — a ceiling-compliant easy run is still contraindicated pre-rebuild.** Return an `impact` directive from weekShape (none <6wk, walk-run 6-12wk) + enforce (block/convert run modality) in enforceShape. Route: week-shape.js + shape-enforce.js.
