...
668. ⬜ **Player CTA can start BELOW THE FOLD on a tall session (found in QA browser walk 2026-07-21).** #638 fixed the overlap (the green "Log set → rest" now sits cleanly above the dots), but on a session with a full set grid the primary CTA is inside the scrollable grid area and can require a scroll to reach — the primary action should ALWAYS be visible. Fix: pin the gp2-gridcta (Skip + Log set/Finish) as a sticky footer of the grid area (or move it out of the scroll), so it's on-screen without scrolling. **Route:** GymPlayer.tsx gp2-gridcta + styles.css. Ties #638/#662.
