...
1002. 🧪 **Coach name "removed" from Profile (JM 2026-07-14, in-app report).** JM's Profile showed a BLANK coach name though the server still held it ("Tadej"). Root cause: the Profile field read ONLY the **device-local cache** (`getSetting('coachName')`, #5000) — a cleared/other/synced-late browser has an empty cache, so it looked deleted even though `user.coachName` was intact server-side. Fix (`src/pages/Profile.tsx`): fall back to the **server value** (`localCoachName ?? user?.coachName ?? ''`) — the source of truth — so the name can never look removed. (The AuthContext hydration stays as a cache warm-up.) tsc 0 · build ✓. **To test (QA):** open Profile → coach name shows "Tadej" (not blank), incl. on a fresh browser / after clearing site data; editing + tab-away still saves. **Route:** UI (Profile).
