...
27. ⬜ **Post-workout flow must be SPORT-DEPENDENT** (extends #26) — ride/run: power/HR/load + legs before/after + fuel/GI; gym: RPE + soreness/pump + volume/top sets + form; yoga/pilates: calm/down-regulation + flexibility + any strain. Mock each per sport (sport toggle).
28. ⬜ **Week strip needs prev/next arrows** — the day strip is stuck on the current week (e.g. 22–28); add ‹ › to page to past/future weeks (Today + wherever the WeekStrip is used).
29. ⬜ **Post-workout mockup tweaks (refine #26/#27)** — (a) button = just **"Save"**; the intervals push is BACKEND, don't surface it in the UI. (b) ADD the intervals **"Feel"** field: 5 faces **STRONG · GOOD · NORMAL · POOR · WEAK** (coloured like intervals), distinct from RPE. Apply when #26/#27 is built.
30. ✅ **Process: proactively update skills when logic changes/improves** — don't wait to be asked. Captured: memory `keep-skills-current` + options-first skill.
