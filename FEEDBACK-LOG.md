...
719. ⬜ **[audit#10] Female menses CUT a quality day — contradicts the engine's own science; phase load-modifier never applied.** Restrict the cut to late-luteal (menstrual only if symptomatic); fold cycleLoadModifier into weeklyLoadBudget (code-enforced). Route: week-shape.js + cycle.js + readiness.js.
718. ⬜ **[audit#9] Bodybuilder assembler under-doses volume ~½ the engine's own 10-20 target (say/do gap).** Scale sets-per-primary + 2nd movement on priority muscles for focus=muscle; unit-test weekly sets/muscle ≥10. Route: gym-split.js assembleGymSession.
717. ⬜ **[audit#8] Concurrent-training separation is PROMPT-ONLY — heavy legs can land ±1 day before a key ride/run.** Add a unit-tested check in reenforceShapeAll detecting heavy gym ±1 day from a quality endurance day → flag/move. Route: server.js reenforceShapeAll.
...
714. ⬜ **[audit#7 SAFETY] Teen gym derates rep COUNT but not LOAD — a minor still lifts ~85% 1RM (enforceTeenGym floors reps<5→5; clampMainReps forces support 3-6 @80-90%).** Add a teen branch capping %1RM (≤70-75%) + a technique band (≥8 reps), overriding the support down-clamp. Route: server.js enforceTeenGym + gym-split.js clampMainReps.
