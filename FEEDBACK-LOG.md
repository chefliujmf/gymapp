...
723. 🔨 [built-QA] **A completed GYM shows a coach review with NO feedback entered AND no review banner (JM 2026-07-22, wife's prod account, "how is this possible? ouch").** ROOT CAUSE — gym feedback lives in the Platyplus store (`plan.feedback` / `activityFeedback[gym-{date}|planId|activityId]`), NOT on the intervals activity like ride/run feel/RPE, so TWO layers had a gym blind spot: the client nag (#679 `feedbackGaps.ts`) excluded gym entirely → no banner; and the coach's `awaitingFeedback` grace (#681, server.js) read the intervals feel/RPE → it never waited for gym feedback and reviewed the session as "none given". FIX (JM chose "Server flag = one source of truth"): pure `server/gym-feedback.js` (`gymHasFeedback` / `gymFeedbackGaps`, unit-tested `src/gym-feedback.test.ts`) read by BOTH the coach grace (awaitingFeedback now checks the gym store for gym-type activities) AND the client banner (new `GET /auth/gym-review-gaps` → ToReviewCard count + ReviewPage rows). Route: server/gym-feedback.js + server.js + src/auth/api.ts + Today.tsx + ReviewPage.tsx.
