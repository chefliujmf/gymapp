...
727. ⬜ **"Weights not logged — tap to add" on a COMPLETED gym opens the START-WORKOUT + reorder player, not a weight-log (JM 2026-07-22, wife's prod account).** DIAGNOSED: `ActivityDetail.tsx:405` `addWeights = () => { setGymSession(gymSessionFromPlan(plan)); navigate('/gym-session/play') }` sends her to the pre-workout GymPlayer ("Plan · reorder · Start workout ~46 min") — wrong for an already-DONE session; she just wants to log the loads she lifted. Needs a post-hoc weight-entry surface (or the player opened in a "log what you did" mode), not a restart. Route: ActivityDetail.tsx + GymSummary onAddWeights + GymPlayer. (UX → mock first.)
