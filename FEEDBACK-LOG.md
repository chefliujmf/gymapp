...
722. 🔨 [built-QA] **Missing-feedback banner only shows on the DAY view — not Week/Month/Schedule (JM 2026-07-22, reported earlier, "must be addressed").** The "you have sessions to review/log feedback" nag renders only in the Day plan view; the Week, Month, and Schedule views have no banner, so a user on those tabs never sees they owe feedback. Show it across ALL plan views (or at the Plan-page level above the tabs). Route: the Plan page + where incompleteFeedback/the banner renders (Today.tsx/plan tabs).
723. 🔨 [built-QA] **A completed workout shows a coach review with NO feedback entered AND no review banner (JM 2026-07-22, wife's prod account, "how is this possible? ouch"; confirmed it's a POST-WORKOUT review).** ROOT CAUSE — the daily review pass reviewed an un-fed-back session DATA-ONLY once the old 1-day grace expired. JM HARD RULE: **"the user MUST enter feedback to get a coach review — there is no way around it."** Feedback ALWAYS lives in the Platyplus store when logged (`plan.feedback` / `activityFeedback[activityId|planId|gym-{date}]`), all sports; the gym also had a nag blind spot (#679 `feedbackGaps.ts` excluded gym → no banner). FIX (code-ENFORCED, not prompt-only): pure `server/gym-feedback.js` `hasSessionFeedback` (universal, unit-tested) is the ONE gate — (a) `POST /api/coach-review` now **409s a review for a session with no feedback** (there is no data-only review, "no way around it"); (b) `awaitingFeedback` uses it (drop the grace-expiry so an un-fed-back session is skipped forever, keep the fresh-window only to avoid double-review); (c) the gym nag banner (new `GET /auth/gym-review-gaps` → ToReviewCard count + ReviewPage rows). Propagated to the daily review-pass prompt + MCP `get_recent_activities`/`save_coach_review` descriptions (synced to host). Route: server/gym-feedback.js + server.js + mcp/server.js + src/auth/api.ts + Today.tsx + ReviewPage.tsx.
725. 🔨 [built-QA] **Remove the manual ⟳ sync button under the check-in (JM 2026-07-22, "don't want people to overuse it, we control the integration").** The check-in card's manual re-pull button (`wchip--refresh`) is gone; the AUTOMATIC re-pull on app focus/visibility + the daily pass still keep HRV/sleep current. Route: src/pages/Today.tsx.
726. ⬜ **Platyplus shows a plan for today but intervals doesn't have it (JM 2026-07-22, "another sync issue").** DIAGNOSED on PROD (LSI7iFKm6zw6/i28814): the 07-22 "Threshold 4×10" plan carries `icuEventId=121594819`, but that intervals event is GONE (404) — while the 07-23 plan's event (121594822, same adapt batch, sequential id) is present. ROOT CAUSE — `healMirror` (#5026, server.js ~2687) only re-pushes plans with **`!p.icuEventId`** (never mirrored); a plan whose intervals event was later DELETED keeps a STALE `icuEventId`, so healMirror skips it and it stays missing forever. (Open q: WHAT deleted 121594819 — orphan-GC vs intervals-side — but the robust fix is a self-healing mirror that detects a gone event + re-pushes.) Route: server.js healMirror + reconcile.
735. ⬜ **A deliberate REST day gets RE-ADDED by the next daily-adapt (JM 2026-07-22, "requested a rest today, now I see an activity again — fix that").** DIAGNOSED via prod audit: 18:05 coach removed today's session (JM's chat rest request), 18:16 the daily-adapt/horizon-fill re-created a session on today ("Z2 High-Cadence Efficiency Spin") because it saw an empty day and filled it. ROOT: a deliberate rest isn't RECORDED, so the adapt can't tell "athlete rested this" from "empty, needs filling" (and #734's fill-to-target makes it worse). IMMEDIATE FIX applied: deleted today's re-added session on prod + QA + intervals (rest restored; today's adapt already ran once/day so it won't recur today). SYSTEMIC FIX NEEDED: make rest STICKY — record deliberate rest dates (UI delete + coach-chat rest request), and the adapt/horizon-fill + create-guard must NEVER fill a recorded rest date. Route: user rest-dates model + buildSystemPrompt/horizonFill/dailyAdapt + upsertPlan guard + a coach tool/UI. Ties #734.
734. ✅ **[NOT A BUG] Coach left Friday empty with 6 workouts/week set (JM 2026-07-22, "investigate if it's a bug").** INVESTIGATED on QA: the week has EXACTLY 6 training days (Mon Easy Spin · Tue Cyclist Strength · Wed Threshold · Thu Easy Base · **Fri REST** · Sat Long Ride · Sun Easy Endurance). `trainingDays=6` is a HARD CEILING (max 6 days/wk, #454 — never more), so 6 filled days + 1 rest = correct; after JM's move-to-tomorrow the coach kept it at the cap and placed the rest on Friday. Working as intended. (If JM wants to train all 7 days, set trainingDays=7.)
733. ⬜ **Notifications center: keep a STORE of notifications (as logs), "Clear all" button, quick access to notification SETTINGS, and notification-TYPE preferences (JM 2026-07-22, Monarch ref screenshot).** New-releases becomes one notification-type preference. "Do we have more types to add?" — inventory the current notification kinds (review, reminder, coach-change, release, push) + propose the preference set. UX → mock first. Route: notifications model (server pushNotification/subkind) + a Notifications panel + Settings.
732. ⬜ **Release notifications ("What's new" with new-feature highlights) don't show anymore (JM 2026-07-22, "only new stuff, we don't mention bug fixes").** src/data/releases.ts latest entry is 2026-06-23 — STALE (no release block added for the #698-#728 batches), and/or the What's-new bell trigger isn't firing. DIAGNOSE the bell's show-logic (lastSeenRelease vs newest) + add release blocks per shipped user-facing batch (the "When you change X" rule already says to). Ties #733 (release = a notification type). Route: src/data/releases.ts + the What's-new bell.
731. ⬜ **No way to switch kg ↔ lbs (JM 2026-07-22, "still don't see how to change from kg to lbs").** DIAGNOSED: there is NO weight-unit preference anywhere in the app (grep found none) — weights are hard-kg. MISSING FEATURE: a units preference (kg/lbs) in Settings/Profile that drives every weight display + the gym weight-entry (#727/#730). Route: a `user.info.weightUnit` pref + a display formatter used everywhere weight shows.
730. 🔨 [built-QA·verified] **Can't edit a COMPLETED workout's weights post-hoc (JM 2026-07-22, "want to change weight post workout, I can't").** Same surface as #727 — the "Log what you did" sheet must ALSO open to EDIT an already-logged gym (change a weight/reps/time after the fact), not just for a no-weights device gym. Fold into #727's build. Route: ActivityDetail/GymSummary → the mode-aware entry, reachable in edit mode + saving back to the log.
729. ⬜ **Gym shows NO load/TRIMP/TSS in QA → how do we measure fatigue & Form? (JM 2026-07-22, "we have a book that defines TSS for gym, not sure it was used").** DIAGNOSED: gym TSS IS computed (`plannedGymTss`, #434 — but a FLAT 45 TSS/h, NOT the RPE-refined `gymTSSfromRPE` in src/tss.ts) and the completed log stores a `tss` (07-21 log had tss:41). THREE gaps: (a) the gym recap/stats never DISPLAY load/TSS (#601 dropped volume; load was never shown); (b) on QA gym load isn't pushed to intervals (read-only) so QA Form/CTL ignore it — QA-only artifact, prod pushes planned gym load; (c) the actual completed load uses a flat/planned rate, not the athlete's real RPE/effort per the "book". Decide the gym-TSS method + SHOW it + confirm it feeds Form. Route: server/icu-steps.js (plannedGymTss/gymTSSfromRPE) + GymSummary/stats display + push path.
728. ⬜ **Coach put a KEY THRESHOLD ride INDOOR in good summer weather for "cleaner ERG power" (JM 2026-07-22, "should have been outdoor; only a thunderstorm would justify indoor").** DIAGNOSED: the venue-inference rule "trainer intervals / bad weather / **short quality → indoor**" lives in THREE layers (`coach-engine-cycling.md:85`, MCP `create_ride` desc `mcp/server.js:199`, + neutral param at :200) and CONTRADICTS the same engine's outdoor-first rule (cycling.md:470 "indoor bike is a LAST RESORT in summer"). The coach followed the wrong one and justified indoor on ERG precision at 22°C/2% rain/26 km/h. FIX: outdoor-first — kill "quality/ERG-precision → indoor"; go indoor ONLY for genuinely bad/unsafe weather or explicit athlete preference. Route: coach-engine-cycling.md + mcp/server.js + (optional) a code guard in upsertPlan. Ties memory platyplus-focus-outdoor-gym.
727. 🔨 [built-QA·verified] **"Weights not logged — tap to add" on a COMPLETED gym opens the START-WORKOUT + reorder player, not a weight-log (JM 2026-07-22, wife's prod account).** [mock approved-pending: mode-aware "Log what you did" — weight×reps / reps / time per exercise mode, prefilled from plan; JM corrected the first mock (not everything is weight×reps).] DIAGNOSED: `ActivityDetail.tsx:405` `addWeights = () => { setGymSession(gymSessionFromPlan(plan)); navigate('/gym-session/play') }` sends her to the pre-workout GymPlayer ("Plan · reorder · Start workout ~46 min") — wrong for an already-DONE session; she just wants to log the loads she lifted. Needs a post-hoc weight-entry surface (or the player opened in a "log what you did" mode), not a restart. Route: ActivityDetail.tsx + GymSummary onAddWeights + GymPlayer. (UX → mock first.)
724. 🔨 [built-QA] **Add a little platypus face to the "Powered by Platyplus" signature (JM 2026-07-22, `[>ಠ ▭ ಠ<]`).** The public activity description's auto-appended brand tagline now reads `Powered by Platyplus [>ಠ ▭ ಠ<]`. Idempotent, server-owned (single source; the coach never writes the tagline). Route: server.js public-text endpoint.

### 🛡️ SAFETY-FLOOR workstream (from #709 audit — code-enforce safety before any new feature; JM 2026-07-22 "do it now")
721. ⬜ **[audit#12] App-wide UX debt: split feedback stores, dead /more route, near-empty Cycling stats gating out manual-FTP, blind navigate(-1) dead-end, off-theme hex (incl. #3d7bff).** Route: ux-flows + nav + stats + admin. Lower priority than the safety items.
720. ⬜ **[audit#11] Gym never progresses week-to-week — no code periodization/deload.** Add a gym meso-cycle (accumulation→intensification→deload) feeding week index into assembleGymSession so reps/%1RM ramp + deload cuts volume ~40-50%. Route: periodization.js + gym-split.js.
719. 🔨 [built-QA] **[audit#10] Female menses CUT a quality day — contradicts the engine's own science; phase load-modifier never applied.** Restrict the cut to late-luteal (menstrual only if symptomatic); fold cycleLoadModifier into weeklyLoadBudget (code-enforced). Route: week-shape.js + cycle.js + readiness.js.
718. 🔨 [built-QA] **[audit#9] Bodybuilder assembler under-doses volume ~½ the engine's own 10-20 target (say/do gap).** Scale sets-per-primary + 2nd movement on priority muscles for focus=muscle; unit-test weekly sets/muscle ≥10. Route: gym-split.js assembleGymSession.
717. 🔨 [built-QA] **[audit#8] Concurrent-training separation is PROMPT-ONLY — heavy legs can land ±1 day before a key ride/run.** Add a unit-tested check in reenforceShapeAll detecting heavy gym ±1 day from a quality endurance day → flag/move. Route: server.js reenforceShapeAll.
716. 🔨 [built-QA, partial: archetype respects mainSport + tri quality-spread; full per-discipline code-enforcement is follow-up] **[audit#6] Triathlete collapses to bike-only in 3 code layers (['triathlon'] matches no sport).** Make archetype/buildSystemPrompt/weekShape triathlon-aware: per-discipline archetypes, all 3 focus lists, ~1 quality/discipline, clamp lowest-priority discipline. Route: archetypes.js + server.js + week-shape.js.
715. ⚠️ **[audit#1 CRITICAL — needs a dedicated careful pass, NOT a rush] Swim intensity is un-enforced.** DEEPER ANALYSIS (2026-07-22): the audit fix ("emit zone→%CSS segments + clamp") is INCOMPLETE — the swim PUSH ignores segments (planToIcuEvent swim uses only distanceM/moving_time/load) AND the athlete-facing prescription is the NOTES text, not segments. Emitting segments would let the clamp fire but change nothing the swimmer does + cause a title/notes mismatch. REAL FIX = server-side refactor: makeSwim sends the structured sets; server clamps each set zone to the shape ceiling (endurance/tempo→2, sweetspot/threshold→3, vo2→5); RE-DERIVE notes+duration+sTSS from the clamped sets (dup makeSwim math server-side). Real breakage risk → dedicated pass with heavy tests, not a marathon-tail rush. Route: mcp create_swim + server upsertPlan swim branch + a new server clampSwimSets.**
714. 🔨 [built-QA] **[audit#7 SAFETY] Teen gym derates rep COUNT but not LOAD — a minor still lifts ~85% 1RM (enforceTeenGym floors reps<5→5; clampMainReps forces support 3-6 @80-90%).** Add a teen branch capping %1RM (≤70-75%) + a technique band (≥8 reps), overriding the support down-clamp. Route: server.js enforceTeenGym + gym-split.js clampMainReps.
713. 🔨 [built-QA] **[audit#5 SAFETY] Postpartum clamps INTENSITY but not IMPACT — a ceiling-compliant easy run is still contraindicated pre-rebuild.** Return an `impact` directive from weekShape (none <6wk, walk-run 6-12wk) + enforce (block/convert run modality) in enforceShape. Route: week-shape.js + shape-enforce.js.
712. 🔨 [built-QA] **[audit#4 SAFETY] Pregnant gym: no supine/T2+ contraindication is code-enforced — assembler resolves flat-supine bench for a T2+ athlete, prompt says "build exactly", overriding the "no supine" rule.** Pass pregnant+trimester into assembleGymSession → filter supine/prone patterns at T2+ + add enforcePregnancyGym save-clamp (peer to enforceTeenGym). Route: gym-split.js + server.js.
711. 🔨 **[audit#3 SAFETY] Postpartum with NO birth-date degrades UNSAFELY to a full build (path only fires if postpartumSince set).** On pregnant true→false, auto-set provisional postpartumSince=today → graded return by default (opt-out not opt-in). Route: server.js PUT /auth/profile + cycle.js.
710. 🔨 **[audit#2 SAFETY] Beginner default week is the MOST aggressive (2 quality, VO2) — "conservative ramp-in" is prompt-only.** Make the conservative shape the DEFAULT with no load history (no CTL/benchmarks / recently onboarded): flat, ≤1 quality, ceiling ≤ tempo, ramp as absorbed load accrues. Route: week-shape.js weekShape.

709. ⬜ **WHOLE-APP RE-AUDIT, scored (JM 2026-07-22).** Re-audit the ENTIRE app in PROD, across ALL sports × ALL personas × ALL accounts, and SCORE the results. Scope: (a) ENGINE LOGIC per sport/persona (dose, zones, periodization, rep-scheme, safety, individualization — per skills `validate-athlete-types` + `coach-simulation`); (b) UX/UI STEPS & PROCESSES end-to-end (plan→player→complete→review, onboarding, logging); (c) STATS pages (per UX best practice — quick access to info, limit clicks + scroll, mobile-first); (d) NAVIGATION; (e) PROFILE; (f) SETTINGS; (g) ADMIN page. Deliver a scored report per area + per persona, with concrete findings + fixes ranked. **Route:** structured multi-area review (likely parallel agents) → scored report artifact. Confirm scope/scale first (large token spend).

706. 🔨 **[AUDITED] Calendar: 07-27 and 08-03 gym have the SAME title (JM 2026-07-22).** AUDIT RESULT: the EXERCISES differ acceptably (07-27 Bench/Shoulder Press/2 Row variants; 08-03 Hip Thrust/Bench/Shoulder Press/Lateral Raise/Cable Row/Pull-up) — the repeated Bench/Shoulder Press are the MAIN lifts and SHOULD recur to progress. The real issue is the identical TITLE "Upper-Body & Trunk Strength" both weeks. The assembler's sessionTitle already varies ("Upper-Body Strength A/B for Cycling") but the COACH overrides it with a generic title (LLM ignores the frame). FIX (todo): code-enforce distinct gym titles across the horizon (append a distinguishing tag / use the assembler title) at save. Route: gym-split.js sessionTitle + upsertPlan/enforce.
705. ⬜ **Remove the "0%" at the end — "0% of what? doesn't make sense" (JM 2026-07-22).** A meaningless 0% shows (stats / feedback progress). Remove it. **Route:** find the 0% (likely the feedback richness bar or a stats figure) and drop it.
704. ⬜ **"Needs attention" is CONFUSING — remove it; coach feedback must be actionable added-value (JM 2026-07-22).** The "Needs attention" label/section in stats is confusing. Remove it and make the coach's stats feedback concrete + actionable (what to do), not a vague flag. **Route:** stats page "needs attention" component + the coach-takeaway content.
703. ⬜ **Stats strength coach takeaway is off-topic — should be about how STRENGTH is progressing GLOBALLY, matching ride/run UX (JM 2026-07-22).** The coach takeaway on the strength stats isn't talking about the right things (global strength progression) and doesn't mirror what we do for running/cycling. **Route:** stats coach-takeaway generation for strength + parity with ride/run.
702. ⬜ **Stats for strength: NO time-range chips selected (6wk/3mo/etc) (JM 2026-07-22).** The range chips aren't selected/active on the strength stats view (bug — other sports have them working). **Route:** Stats page range-chip state for strength.
701. ⬜ **Planned gym workout UX ≠ ride/run planned UX (JM 2026-07-22).** The planned strength detail doesn't follow the same UX as a planned cycling/running workout. Make it consistent. **Route:** CoachPlanDetail (gym vs ride/run planned rendering) — parity audit.
700. ⬜ **Gym HR not shown like other activities (JM 2026-07-22).** #695 added a "90 bpm avg" chip, but ride/run show HR in the HERO row — gym should present HR the SAME way as running/cycling, not just a chip. **Route:** ActivityDetail gym branch / GymSummary HERO to include HR like ride/run. Ties #695.
699. ⬜ **Remove the "Next" section from the coach review (JM 2026-07-22, "as I said earlier").** The completed-activity coach review still shows a "Next:" block; JM wants it removed. **Route:** CoachVerdict / ActivityFeedback review render (drop the `next` display) + stop the coach emitting it, or hide it.
698. ⬜ **Coach must not mention "pump" / "no pump" in feedback — not relevant anymore (JM 2026-07-22).** #660 removed the Soreness/pump field, but the coach review text still references "zero pump / no pump". Scrub/stop it. **Route:** coach review prompt (triggerActivityReview/reviewMsg) + a scrub if needed.

696. 🔨 **[built — QA, promoting] Planned gym showed 42 min but the description said "55-60 min" and the real session was ~54 (JM 2026-07-21, screenshot).** TWO bugs: (a) `estimateGym{Minutes,Seconds}` defaulted rest to a flat **60s/set** — heavy low-rep strength needs 2-3 min between sets, so a ~55-min session estimated at 42. Now rest **scales with rep count** when not explicit (≤6 reps→150s, ≤12→90s, else 60; timed holds→1 set/15s). Sample heavy session now lands 55 min. Client `src/plan.ts` + server `server/icu-steps.js` kept IN SYNC + tests. (b) The description **restated the duration** ("About 55-60 min") — JM: "why have it in the description?" New `stripGymDurationProse` removes a QUALIFIED session-length phrase but KEEPS fuelling timing ("snack about 60-90 min before") via a before/after lookahead; applied in upsertPlan + the reenforceShapeAll sweep. Unit-tested. **Route done:** icu-steps.js + plan.ts + gym-split.js + server.js.
695. 🔨 **[built + VERIFIED QA, promoting] Gym showed no HR though intervals has it (JM 2026-07-21, screenshot: "Afternoon Weight Training @ 90bpm").** ROOT CAUSE: the session is in intervals TWICE — the Coros **"Afternoon Weight Training"** (`i167928014`, HR 90, `strava_id`) AND an empty **STRAVA duplicate** (`19408262163`); Platyplus linked to the HR-less duplicate. FIX: the gym view finds the day's device strength activity that actually HAS HR and sources it from there → **90 bpm avg chip now shows** (verified in Chrome). **Route done:** ActivityDetail.tsx `deviceHr` (fetch day's activities, pick the strength one with HR). Follow-on: the two intervals cards should de-dupe/pair (#666b territory).
686. ✅ **[Strava title — RESOLVED + backfilled] Coach didn't set the gym's Strava title/description (JM).** DIAGNOSIS: intervals→Strava write-back WORKS (JM's ride "Easy Z2 Endurance" + "Powered by Platyplus" is live on Strava); the coach flow now DOES call set_activity_text for gym (proven by the regen). This one session was stale (reviewed pre-fix). BACKFILLED on prod (JM-authorized, via his api_token through the box → app public-text endpoint): `i167928014` (the Coros activity carrying `strava_id`) now reads **"Full-Body Strength"** / "Light recovery strength session, all movement patterns. Powered by Platyplus" → syncs to Strava. NOTE: a direct external-Strava write is classifier-blocked from the Mac; done via the SSH prod-ops channel (memory `platyplus-operate-prod-directly`). Future gyms set the title automatically.
694. 🔨 **[built — QA] GYM activity detail showed RIDE metrics — "power", "GPS", "No GPS or sensor data" — and no session content (JM 2026-07-21: "today's workout mentions power and gps, it's a gym… still no HR").** ROOT CAUSE: ActivityDetail treats any non-run/non-swim activity as a RIDE — HERO = `['Load','Norm power','Intensity','Avg HR']`, bottom = ride/run tabs → "No GPS or sensor data". A gym's real data (exercises/sets/reps/time) lives in the Platyplus **workout log**, not the (near-empty) intervals stub, so the page showed ride scaffolding over nothing. FIX: for a gym activity, render the same **GymSummary** the post-workout page uses (exercises · sets · reps · time · HR-if-present · PRs) and NONE of the ride power/GPS scaffolding. The log is fetched from the **server** (`GET /auth/logs`, new `authApi.serverLogs`) so it shows on ANY device, not just the phone that recorded it (Dexie is per-device — that's why JM's browser saw nothing). **HR note (JM's "still no HR"):** verified the data — this session's intervals activity (19408262163) is a bare STRAVA stub with **no HR/type/duration**, and the gym log has **no HR samples either** (the in-app player didn't record HR, and the Coros HR never reached intervals). So there is genuinely no HR to show; GymSummary now shows HR only when present (honest), and shows the real session instead. The deeper "get Coros HR into the gym" is a separate sync gap. **Route done:** ActivityDetail.tsx gym branch + GymSummary reuse + authApi.serverLogs + server GET /auth/logs (already existed). Ties #666a, #686.
     VALIDATION: 798 tests + tsc green; QA browser next.

693. 🔨 **[built — QA] SAFETY: `PUT /api/activity/:id/public-text` was missing the `IS_STAGING` guard (found 2026-07-21 during the review-regen test).** Every other intervals write is guarded `!IS_STAGING` (QA shares the REAL prod athlete i28814, activity ids are GLOBAL), but this one wasn't — so a QA coach review calling `set_activity_text` would rewrite the athlete's ACTUAL Strava title/description. FIX: on QA, skip the intervals write and return `{ok, staged, ...payload}` so the coach's tool still "succeeds" and the intended text is testable; only PROD publishes. Verified in the review-regen test: the coach generated "Full-Body Strength" + a plain description targeting activity 19408262163, and it was STAGED (no intervals PUT, real Strava untouched). **Route done:** server.js public-text endpoint IS_STAGING guard. Ties [[platyplus-orphan-gc]] (QA read-only toward intervals), #686.
     VALIDATION PASS (2026-07-21, "test like crazy + quality skill"): gym engine #691/#692 → persona matrix (cyclist legs-first, runner single-leg+plyo, swimmer pull+shoulder, tri, bodybuilder, strength, health) + 3 new unit tests (798 green). #677/#678 → QA Chrome: Cancel works + discards + no re-review; **caught + fixed a button-layout bug** (`.btn width:100%` made Cancel full-width / Save a sliver → width:auto). #660 Soreness/pump gone confirmed in-app. #681/#686 → regenerated JM's gym review on QA: honest 7/10, next=objective (not a workout list), clean public text, safely staged. Promoted via PR #236.

692. ⬜ **Sport-emphasis: RUNNER should be SINGLE-LEG + PLYOMETRIC, not the same as a cyclist (JM 2026-07-21, from his emphasis question).** #658 gives the runner the SAME priority patterns as the cyclist (['squat','hinge'] heavy bilateral) — but running is SINGLE-LEG, so a runner's gym should structurally PRIORITISE single-leg strength (split squat, step-up, single-leg RDL) + a weekly PLYOMETRIC/reactive slot (pogos/bounds → tendon stiffness → running economy, Beattie/Blagrove) + hip/glute frontal-plane stability. Currently that's only in the SPORT_EMPHASIS `cue` TEXT (prompt), not enforced in SELECTION — so the assembler picks the same bilateral moves as a cyclist. FIX: give the runner emphasis its own priority selection — prefer the unilateral variant of squat/hinge (Bulgarian/step-up/SL-RDL) + add a plyo/reactive slot; similarly make the SWIMMER emphasis explicitly include a shoulder-HEALTH move (external rotation/scap) not just pull. **Route:** gym-split.js SPORT_EMPHASIS + assembleGymSession (per-sport pattern/variant selection). Ties #658/#687. (Bodybuilder correctly gets no bias; cyclist correct = heavy bilateral legs.)

691. ⬜ **Group same-EQUIPMENT exercises together in a gym session when it fits the objective (JM 2026-07-21, "for after what we're doing", not-hard).** Order the main set so moves sharing equipment are adjacent (all dumbbell, then barbell, then cable/machine) to minimise station-switching — WHEN it doesn't break the training logic (don't group if it forces two heavy same-pattern lifts back-to-back or hurts recovery ordering). A light ordering pass in the assembler (`assembleGymSession`) after selection: stable-sort the main set by equipment while preserving the primary-first / pattern logic. Low effort. **Route:** gym-split.js assembleGymSession (equipment-aware stable sort). Ties #687.

**★ #687 STATUS (validated on QA 2026-07-21): the assembler is UNIT-proven but the prompt-injection did NOT take effect end-to-end — the coach LEFT the old broken 07-27 gym (Dead Bug in warm-up AND main #685, two rows #683, glute-bridge-as-reps #684). SAME "prompt-only gets ignored" lesson. The assembled frame is NECESSARY but NOT SUFFICIENT. THE REAL FIX = ENFORCE it in code: a save-time gym guard that FIXES the session to match the assembled structure (dedup exId/name across ALL sections, demote a 2nd same-pattern main to accessory, force a hold-tagged exId to timed, reject/repair a mobility-only move tagged main), applied in upsertPlan + a gym re-assemble sweep in reenforceShapeAll — OR have the code BUILD + SAVE the gym directly (not ask the coach). This is the NEXT build (the enforcement half of #687). Route: mcp/gym-guard.js + server.js upsertPlan/reenforceShapeAll + gym-split.js.**

690. 🧪 **QA staging deploy FAILED on a Docker container-name collision (JM 2026-07-21).** The `gymapp-staging` container was OOM-killed (Exited 137 — memory pressure from rapid deploys + coach adapts), and the next deploy couldn't recreate it because the name was still held by the exited/mangled container ("name is already in use … to reuse that name"), leaving QA DOWN. FIX: cleared the stuck containers on the box (`docker rm -f`) + re-ran the deploy → recovered (healthy, 200). Hardened `scripts/deploy-staging.sh` with `--remove-orphans` so a stale container can't block a future deploy. Watch the deeper cause: exit-137 = the box is memory-tight (staging + prod + db×2 + coach LLM tasks) — may need a memory limit / swap review. **Route:** deploy-staging.sh (done) + box memory. ✅ QA recovered + hardened (dev).


**★ 2026-07-21 JM feedback batch (day-view + gym + feedback UX — surfaced while testing; ADDRESS next pass):**

687. ⬜ **★ WORLD-CLASS GYM WORKOUT ASSEMBLER — build each session from the exercise LIBRARY in CODE, tailored to the user (JM 2026-07-21, DIRECTION: "I don't want Centr templates; I need the coach to CREATE the workout, adapting to the user's context — world-class, tailored to the person, from the list of exercises").** This is the coherent fix for #675/#683/#684/#685/#686 and the answer to "why can't the coach follow the books" (selection/sectioning/mode/title were prompt-only → ignored). Build a CODE ASSEMBLER (extend `assignWeeklyGym`) that DYNAMICALLY composes a session from the catalog — NOT canned Centr plans — adapting to the athlete's CONTEXT: sport-emphasis (#658), focus→rep-scheme (#648/#649), OWNED equipment (#645), recent history (variety/skip-recent), readiness/phase, injuries. It must, IN CODE: (1) SECTION correctly — pick warm-up from a mobility/activation pool + cool-down from a stretch pool + main from strength patterns (section-appropriate, not a stretch as a main or a squat as a warm-up); (2) ONE PRIMARY per movement pattern, accessories flagged (kills #683); (3) DEDUP the same exId/name across the whole session incl. across sections (kills #685); (4) set MODE from the exId `timed` hint — holds timed, lifts reps (kills #684; + fix derive-type for "glute bridge"/isometric-by-default moves, #664); (5) a DISTINCT descriptive title per session (kills #675); (6) hand the coach a FILLED frame (exId+section+mode+load-target pre-set) so it only authors cues/tips — code decides structure/selection, LLM authors within it (the proven pattern). Backstop: harden `gym-guard.js` (dedup + one-primary-per-pattern + section-appropriateness + mode-consistency). **This is the BIG next build.** **Route:** gym-split.js (assembler + WARMUP_POOL/COOLDOWN_POOL) + gym-guard.js + mcp/create_workout (consume the frame) + derive-exercise-type.mjs + coach-engine-strength.md + tests. Ties #636/#648/#658/#663/#664. Skill validate-athlete-types (tailored to WHOEVER signs up).

686. 🔨 **[largely addressed by #666b — verify on QA] Coach did NOT set a public TITLE + DESCRIPTION for a completed GYM activity → Strava shows the generic Coros "Afternoon Weight Training" (JM 2026-07-21, prod Strava screenshot).** DIAGNOSIS: set_activity_text already handles gym ("Full-Body Strength" is a documented example) and `sportGroupOf('WeightTraining')→'gym'`, so the machinery is fine. The real cause was the review often didn't have the intervals activity id for gym: the device (Coros) WeightTraining activity wasn't linked at review time. **#666b fixes the core** — the completed device activity is now PAIRED + activityLinks-recorded at completion, and the feedback endpoint's sibling-resolution + triggerActivityReview pass that id, so the review's set_activity_text targets the real activity. RESIDUAL: device→intervals sync can LAG past the in-app finish, so the immediate review may still lack the id — but the DAILY review pass backstops it (get_recent_activities shows the now-synced activity `reviewed:false` → the coach reviews + sets public text). Net: should now set a clean gym title/description on Strava. **Verify on QA:** finish a gym, wait for the device activity to sync, confirm the coach sets a plain title + description (privacy-scrubbed). Ties #666b, #674, #650, #681.
686-OLD. ⬜ **Coach did NOT set a public TITLE + DESCRIPTION for a completed GYM activity → Strava shows the generic Coros "Afternoon Weight Training" (JM 2026-07-21, prod Strava screenshot).** The coach is instructed to call set_activity_text (public-safe title + description) for a matched device activity, but for JM's gym it set nothing → Strava keeps the device default, no description. Verify: does the completion/review flow actually invoke set_activity_text for GYM device-matched activities? (It may only fire for ride/run, or the review ran before the activity id was resolvable.) The public text should be a clean session title (e.g. "Full-Body Strength") + a one-line plain description — respecting the #650 privacy scrub. **Route:** the completion→review→set_activity_text path (server.js triggerActivityReview/triggerPlanReview + mcp set_activity_text) for gym. Ties #674 (title voice), #650 (privacy), #681 (review flow).

685. ⬜ **SAME exercise in WARM-UP and MAIN set — "Dead Bugs" 45s in the warm-up AND 3×10 in the main (JM 2026-07-21, "ayaye").** An exercise must not appear in two sections of the same session (a warm-up move ≠ a main-set move). Part of the same gym exercise-SELECTION defect class as #683 (near-duplicate rows) + #684 (mode mismatch) — the coach picks redundant/overlapping exercises. **PATTERN: gym exercise selection needs a coherent CODE guard** (peer to gym-guard warmup/cooldown/unilateral): reject a session that (a) repeats the SAME exId/name across sections, (b) has two "main" moves of the same movement pattern (#683), or (c) prescribes a hold-tagged exId as reps (#684). One place, enforced, not prompt-only. **Route:** gym-guard.js (dedup-across-sections + one-primary-per-pattern + timed-consistency checks) + create_workout guidance. Ties #636/#658/#663/#664/#683/#684 — the gym-selection rework.

684. ⬜ **Exercise mode/video MISMATCH — "Glute bridge 1×12" but the demo video is a HOLD/pause like a plank (JM 2026-07-21, wife's Jul 27 gym).** The session prescribes glute bridge as 12 REPS but the catalog video shows an isometric HOLD (the exId is likely a "Glute bridge hold" variant). A hold move must be prescribed TIMED, and the demo must match the prescription. #664 stamps `timed` on holds — the coach must use it (a "hold" exId → timed, not reps), and the player/detail must not show a reps count under a hold video. **Route:** #664 hint adoption (create_workout defaults mode from the exId's `timed`) + verify the glute-bridge catalog entry's name/video/mode agree. Ties #664/#642.

683. ⬜ **Redundant near-DUPLICATE exercises in one gym session — "Bent Over Two-Dumbbell Row" 3×8 AND "One-Arm Dumbbell Row" 2×10 (JM 2026-07-21, wife's Jul 27 gym): "same exercise".** Both are horizontal ROWS (same movement pattern) in the same session — the coach picked two variants of one pattern instead of covering distinct patterns. The gym balance/variety should not schedule two of the same pattern as separate "main" slots (a 2nd same-pattern move is only OK as a deliberate accessory, not a duplicate main). Enforce ONE primary per movement pattern per session (patternFromExercise already exists — flag/collapse duplicates). Ties #636/#658/#663 (patternFromExercise), the gym-guard. **Route:** gym-guard.js / gym-split coverage check + create_workout guidance.

682. ⬜ **Day-view card THUMBNAIL doesn't match the real workout structure (JM 2026-07-21, wife account — "to be done later").** The small thumbnail on the plan CARD (Today/day view) shows a generic bar pattern that doesn't represent the session; the DETAIL view's "PLANNED PACE by zone" chart shows the TRUE structure (easy run + 6 strides = yellow spikes near the end). The card thumbnail should be generated from the real segments (like the detail chart / PowerBlocks), so the mini-preview matches the actual workout shape. **Route:** the plan-card thumbnail component (Today.tsx / the card renderer) — derive it from segments like the detail PLANNED-PACE chart. Ties [[platyplus-activity-view]] (thumbnail=PowerBlocks from real stream).

681. 🔨 **[built — QA] Coach REVIEWED the workout but there was NO user feedback yet (JM 2026-07-21, wife's session — "another big problem").** FIX (chose a clean hybrid of a+b): a GRACE window so the review WAITS for the athlete on a fresh session, then falls back to a data-only review if they never log. The coach's `get_recent_activities` now returns `awaitingFeedback:true` for a session that's today/yesterday, not yet reviewed, and has no feel/RPE anywhere (server.js ~3252) — and the daily REVIEW pass (`reviewMsg`) + the MCP tool desc tell the coach to SKIP those (reviewing now beats the athlete's input and double-reviews when they submit). Past the grace with still no feedback, the flag drops → the daily pass reviews data-only, ONCE. The athlete's submit drives the (single) review while fresh. No double-review: on-submit review is also gated to fire only when the feedback actually CHANGED (#677). **Route done:** server.js get_recent_activities `awaitingFeedback` + reviewMsg skip + mcp/server.js desc + the #677 change-gate.
681-OLD. ⬜ **Coach REVIEWED the workout but there was NO user feedback yet (JM 2026-07-21, wife's session — "another big problem").** The activity shows a full coach review (Score 7/10, verdict, Next) while "How did it go?" is still EMPTY/unsubmitted. The review is firing on COMPLETION (device-matched) instead of waiting for the user's feel/RPE — so the coach reviews with no user input, then STILL asks the user (whose submit would trigger a SECOND review). Decide the intended flow: either (a) the coach reviews once on completion using available data and the "How did it go?" is optional enrichment (then don't re-review on submit, ties #677), or (b) the review WAITS for user feedback. Pick one, make it consistent, and don't double-review. Also visible on this PROD screen: "Soreness/pump" field still present (#660 removal is on QA, not prod). **Route:** the completion→review trigger + ActivityFeedback. Ties #659/#677/#660.
...
678. 🔨 **[built — QA] Editing SETS — no way to DISMISS/cancel (JM 2026-07-21).** FIX: GymSetEditor now takes `onCancel` — an explicit **Cancel** button (beside Save) that DISCARDS local edits and closes; Save is disabled until something actually changed (`dirty`). PostWorkout wires onCancel → close. **Route done:** GymSetEditor.tsx + PostWorkout.tsx.

677. 🔨 **[built — QA] Editing "how it went" feedback — can't DISMISS; Save re-triggers a coach review → LLM overuse (JM 2026-07-21).** FIX (both client + server, defence in depth): ActivityFeedback now snapshots the saved feedback on Edit → a **Cancel** button discards edits, and Save SKIPS re-posting when nothing changed. The SERVER also gates both review triggers (`/auth/activity/:id/feedback` + `/auth/plan/:id/feedback`) on a content signature (feel/rpe/fields/note) so an unchanged re-save NEVER burns a coach review regardless of client. Ties [[platyplus-coach-token-thrift]]. **Route done:** ActivityFeedback.tsx (snapshot/cancel/unchanged-gate) + server.js fbSig change-gate on both feedback endpoints.
...
**★★ 2026-07-21 FULL COACH ADAPTATION AUDIT (JM: "are you fixing this for ALL our variables — activity, individuals, metrics? critical for this to work"). 4 parallel agents: per-sport · athlete-type · metrics-utilization · SAY=DO. VERDICT: the "code-enforce it, don't just prompt it" lesson (#620/#648) is only HALF-APPLIED — safety rails (day/week caps, ride/run intensity ceiling + quality count, teen 1RM, pregnancy SHAPE, easy clamp) ARE code-enforced on every path; but an ENTIRE dimension (gym rep-scheme/balance/equipment, weekly load band, swim ceiling, variety, periodization) + the beyond-FTP metrics are INJECTED-not-ENFORCED, so the LLM can silently violate them. Skill `validate-athlete-types` upgraded to the world-class + code-enforced-vs-prompt-only gate. Roadmap #649-#657 below, ranked by risk.**

661. ⬜ **App RE-ASKS for gym feedback JM already entered (JM 2026-07-21).** HYPOTHESIS (needs confirming WHERE he saw it): gym feedback is stored in the PLATYPLUS feedback store keyed `gym-<date>` (via `authApi.activityFeedback`), but the "needs feedback" nag (`feedbackStatus`/`incompleteFeedback` → Today "to review" + `ReviewPage`) reads `feel`/`icu_rpe` off the INTERVALS activity object (`src/feedbackGaps.ts:25-31`) — so a gym session that also exists as an intervals activity (device-recorded/matched) is flagged "needs feedback" forever even though the Platyplus feedback IS entered. Alt cause: a feedback-KEY mismatch across views — the PLAYER's `gymFeedbackKeys` altIds omit planId+activityId, so feedback saved in one view isn't found in another (the 4 call sites produce different canonical ids: player=`gym-<date>`, PostWorkout/ActivityDetail=planId). **FIX (once the surface is known):** make `feedbackStatus` gym-aware (consult the Platyplus feedback store for a gym session, not just intervals fields) and/or unify the feedback keys so every view's altIds include `gym-<date>` + planId + activityId. **Route:** feedbackGaps.ts + plan.ts gymFeedbackKeys + the nag call sites. Ties #659, the #feedback-key-audit.

660. 🧪 **Removed "Soreness/pump" from gym feedback — jargon nobody understands (JM 2026-07-21).** Dropped the `Soreness/pump` field from `GYM_FIELDS`; gym feedback is now just how-it-felt + RPE + Form + Pain/Niggles. Safe — it wasn't in `ICU_FIELD_CODES` so it never round-tripped to intervals (only Pain/Niggles does). Ties [[no-captain-obvious]], the plain-language rule. **Route:** src/icu-fields.ts. ✅ built (dev).

659. 🧪 **Completed gym showed "Retry coach review" while the review was just PENDING (JM 2026-07-21 "do not show a retry if we are just waiting for it and there were no errors").** Root cause traced on prod: the workout LOGGED fine (54 min, 23 sets) and the coach review GENERATED with full content — but the review is async (~1-2 min) and `ActivityFeedback` only polled AFTER a feedback submit / manual Retry, never on a fresh completion. So the summary rendered before the review landed, saw none, and flashed the "🔁 Retry coach review" (a give-up state) even though nothing had failed. FIX: `awaitReview` prop threaded GymPlayer(done) → GymSummary → ActivityFeedback; on mount, if no review yet + awaitReview, **auto-start the review poll** → shows "🔎 Your coach is reviewing this session…" and fills in when it lands; Retry now appears ONLY after the poll genuinely gives up (a real error/timeout). **Route:** ActivityFeedback.tsx + GymSummary.tsx + GymPlayer.tsx. ✅ built (dev). *(Related, not yet fixed: the pre-start ESTIMATE read ~73 min (crude estSec, 75s/set) vs the 54 min actually logged — cosmetic over-estimate; and if the SCHEDULE still showed the session un-ticked that's the #644 stale-cache. The completed review ALSO named tomorrow's + next week's sessions in `next` — a #625 violation to watch.)*

**★★★ 2026-07-21 FULL REVIEW (JM "full review — UX/UI flow, exercise model, coach prescription per athlete; use chrome"). 3 parallel audits, all grounded in file:line. Scores: UX flow 3-4/5 · exercise model 2/5 · coach prescription 3.5/5 (world-class). The code-enforced core (#648/#649 dose/rep-scheme/teen/pregnancy-shape) is CONFIRMED real on every save+sweep path. Remaining gaps below (#663-#665 new; the prescription top-8 mostly refine #650/#652/#654/#657/#658). Browser: confirmed timed warm-ups (#642) + completion state render correctly on QA.**

672. ⬜ **Fartlek/surge relabel doesn't clean the DESCRIPTION prose (found verifying #671, 2026-07-21).** When shape-enforce relabels a contraindicated "Fartlek Run" → "Tempo Run" for a pregnant/maintenance athlete, it fixes the TITLE + clamps the SEGMENTS (executed intensity is safe), but the coach-authored DESCRIPTION still reads "31 min easy run with 5 unstructured effort bursts by feel" — a title/description MISMATCH that still describes contraindicated surges. LOWER severity than the privacy leak (#671, fixed — what actually runs is clamped), but the prose should match the relabeled title. **Fix:** when relabeling a surge/fartlek session on a capped week, also strip/neutralize the surge language in objective/notes (a prose scrub peer to scrubPrivate), or regenerate the description. **Route:** shape-enforce.js honestTitle path + reenforceShapeAll. Ties #632/#671.

671. 🔴 **SAFETY — PREGNANCY PRIVACY LEAKS + CONTRAINDICATED work on STALE plans (found in QA "test like crazy", Xenia pregnant runner, 2026-07-21).** Xenia's future QA plan still had "**pregnan…**" in 4 session OBJECTIVES (Jul 21/28, Aug 2/9 — "volume trimmed to [account for pregnancy]") AND a "**Fartlek Run**" (Aug 4 — "5 unstructured effort bursts by feel"), which JM explicitly forbids in pregnancy. ROOT: the #650 privacy scrub + the shape-enforce surge/fartlek relabel run at SAVE time (new writes) but there was **no PRIVACY sweep over STALE plans** — a pregnant athlete's pre-#650 plans keep leaking "pregnant" into descriptions that sync intervals→Strava, and the fartlek relabel only reaches a plan the adapt re-touches. **FIX (#671):** added a privacy sweep to `reenforceShapeAll` — scrub title/notes/objective/success of EVERY future plan + re-push changed ride/run/swim so the PUBLIC copy is clean (not just the DB). (The fartlek relabel is already in the sweep via `enforceShapeIntensity` — it fixes stale plans once the sweep runs.) ⚠️ Both depend on the sweep actually RUNNING — reenforceShapeAll fires at the end of `runDailyAdapt` (3697); confirm it completes (the QA adapt is slow/#608 — verify on a completed run). **BLOCKER — do NOT ship any pregnancy-facing path until a completed adapt is confirmed to scrub + relabel Xenia's stale plans.** **Route:** server.js reenforceShapeAll. Ties #650/#632/[[platyplus-readiness-model]] privacy.

670. ⬜ **Concurrent daily-adapts race on the in-memory store — one can revert the other's save (found in QA "test like crazy" 2026-07-21).** Triggering two `/api/coach/daily-adapt` runs for the same athlete overlapped: both loaded `store`, one clamped stale gym mains → save(6), the other saved a stale in-memory copy back → 8. The `[gym-reps] … clamped 4` log fired twice, but the DB transiently showed 8 until the races settled (final state correct = 6). LOW RISK in prod (the scheduler serializes — one adapt per athlete per tick, #469), but the manual endpoint + any overlap can race. **Fix:** a per-user adapt LOCK (skip/queue if one is already running for that user) around `runDailyAdapt` / the manual endpoint. **Route:** server.js runDailyAdapt + /api/coach/daily-adapt. NB this was a TEST ARTIFACT — the gym rep-scheme sweep itself is VERIFIED working (support_build mains 8→6, accessory One-Arm Row kept 10, on the real adapt path).

669. ⬜ **Development priority is WEAKNESS-only, not GOAL-aware (JM 2026-07-21 "we target weakness IF it fits the goal — for an all-rounder yes, for another objective it's different; how do we define that?").** `athleteProfile.focus` (`perf-metrics.js:63-85`) is computed PURELY from metrics (TTE/W′/EF/rider-type) with ZERO reference to the objective → it always attacks the generic limiter. Right for an all-rounder, WRONG for a specialist (a crit sprinter gets pushed on long threshold — their weakness — when the event needs repeatable anaerobic + sprint; a TT-er should keep hammering threshold even though it's their strength). **DESIGN — priority = (GOAL/EVENT DEMANDS) ∩ (weakness relative to that demand) + MAINTAIN goal-critical strengths:** add a `GOAL_DEMANDS` map (objective/focus → a demand vector: threshold·VO2·anaerobic/W′·durability·sprint) — TT/fondo/climb → threshold+durability; crit/track → anaerobic+neuromuscular+repeatability; road race → threshold+VO2+sprint; 5-10k → VO2+threshold; marathon → threshold+durability+fat-ox; general fitness → balanced (raise the limiter). Then `athleteProfile.focus` intersects the demand vector with the metric profile: demanded+weak → ATTACK, demanded+strong → sharpen/maintain, weak-but-irrelevant → SKIP. Feeds `# DEVELOPMENT PRIORITIES` (already computed from focus) so the plan trains what matters FOR THE GOAL. Science: event-demand periodization (Friel; Coggan rider-type × event; Seiler). **Route:** perf-metrics.js computeAthleteProfile (goal input) + server.js # DEVELOPMENT PRIORITIES + docs. Ties #622/#624/#652, [[platyplus-beyond-ftp-metrics]], [[platyplus-numbers-not-categories]]. **The real "make me better at what matters" fix — pairs with #652.**

667. ⬜ **Sleep-need stat: confusing rounding copy + manual value mismatch (JM 2026-07-21, "9.2 rounded is not 9.25 lol; if I aim for 9.25 why show 9?"). FIX LATER (after the gym rework).** The card averages best-recovery nights to 9.2 h then rounds to the nearest quarter-hour = 9.25 h (mathematically the nearest 0.25, but the COPY "averaged 9.2 h — rounded to a 9.25 h target" reads like broken rounding to a user). AND the "Your value" manual input shows **9** while the Auto/displayed value is **9.25** — inconsistent. Fixes: (a) reword so the round-to-quarter-hour is clear (or round to a friendlier 0.5), (b) seed the manual input from the same computed value so it doesn't show a stale 9. **Route:** the sleep-need benchmark card + its rounding/estimate (likely `src/Benchmarks.tsx` / learnedStat / a sleep-need computation). Ties [[benchmarks-manual-vs-computed]].

666. ⬜ **Planned + Coros-completed gym = TWO activities in intervals, not merged; Platyplus completed shows the coach review but NO HR (JM 2026-07-21).** Coros can't START a planned workout from the watch, so it uploads a FRESH activity → the Platyplus-planned gym event and the Coros activity sit UNPAIRED in intervals (looks like 2), and Platyplus's completed view has the plan's coach feedback but never pulls the Coros HR/streams. JM wants ONE merged entry both places: the completed activity LINKED to the planned, with HR, and no duplicate in intervals. **DESIGN:** on gym completion / reconcile, when a device activity matches a same-day Platyplus-planned gym (`icuFindMatch` already detects it → "matched"), (a) LINK: mark the plan completed with the activity id (done) + PULL the activity's HR/duration/streams into the Platyplus completed session (currently missing — that's the "no HR"); (b) DE-DUPE in intervals: since gym events are deep-link-only (no workout model to auto-pair), DELETE the redundant Platyplus-planned event once matched (or write the plan deep-link into the Coros activity's description) so only ONE entry remains. prod-only intervals writes (IS_STAGING guard). **Route:** completeActivity / reconcileFromIcu match path + the gym summary HR pull + intervals event GC. Ties [[platyplus-orphan-gc]], [[platyplus-plan-ownership-intervals]], #123 match-first.

665. ⬜ **UX/UI FLOW — 12 defects (JM full review, 2026-07-21).** (1) 🔴 JARGON on beginner-facing surfaces: "Target TSS / Target IF 0.85 / est FTP / e1RM" unglossed in `CoachPlanDetail.tsx:164-177`, bare TSS+Form on plan cards `Today.tsx:372,236`, acronym-soup Stats subs `hubs.tsx:64` — add InfoDots / plain labels (tempo already has a good one). (2) 🔴 OFF-THEME hardcoded hex `#12180f`/`#26421f` completed-gym banner `CoachPlanDetail.tsx:219` (breaks light mode) → theme tokens (skill platyplus-theme). (3) SPLIT feedback model — non-gym `/feedback/:id` saves via `planFeedback` (plan id) while gym/device go via `activityFeedback` (activity key) → same ride two stores (`PostWorkout.tsx:79` vs `ActivityFeedback.tsx`); ties #661. (4) native `confirm()` popups mid-workout (`GymPlayer.tsx:390,392`, Ride/Run players) → in-app sheet. (5) home lands on cramped MONTH grid not the Day/check-in screen (`Calendar.tsx:56`). (6) stale copy "Open it from Today" (nav is Plan) `PostWorkout.tsx:45`. (7) deactivated Eat/Mind still advertised in hubs (`hubs.tsx:118`) → gate behind the off-flag. (8) 4 different empty-day strings (Today vs Calendar) → standardize. (9) gym stage blank black box on missing media (`GymPlayer.tsx:69`) → 🏋️ emoji fallback. (10) refresh/deep-link dead-end on `/gym-session/play` (`GymPlayer.tsx:270`) → persist by plan id. Browser-confirm: #3 round-trip, #4 feel, #5 mobile, #10 repro. **Route:** per file above.

664. ⬜ **EXERCISE MODEL has NO type — root cause of #642/#645-class (JM full review, 2026-07-21; exercise audit 2/5).** mode(reps/timed)/eachSide/loaded are 100% LLM-GUESSED per exercise: catalog has 0 type fields; `create_workout` params are all optional with NO server default/coercion; `gym-guard` eachSide list misses 341 unilateral moves (lunge/step-up/pistol/curtsy/skater/windmill/get-up) and there's NO timed guard (nothing rejects a plank sent as reps). Counts: 1,144 should-be-timed (614 with no `seconds` at all), 18 loaded-tagged-Bodyweight (goblet/sandbag/slam-ball/sled/prowler/yoke/farmer), 341 unilateral unguarded. **THE ONE FIX (code-decided, token-free):** a derivation pass in `scripts/build-catalog.mjs` stamping every entry with `timed` (category Mobility/Yoga/Pilates + name plank/hold/isometric/hang/wall-sit/carry/stretch/pose) + a default `seconds`, `eachSide` (unilateral name set), `loaded`/corrected equipment (external-load name set) → then `create_workout` DEFAULTS mode/eachSide/weight-trackability from the `exId` hint (LLM overrides allowed), `gym-guard` reads the hint + gains a timed guard, player defaults from it. Regenerates exercises.json + media re-sync. **Route:** build-catalog.mjs + mcp/server.js + gym-guard.js + coach-engine-strength.md + gym-split.js + tests. Ties #642/#647e/#639/[[platyplus-coach-token-thrift]].

663. 🧪 **BUG in #649 — clampMainReps over-clamped ACCESSORIES (found by the prescription audit, 2026-07-21).** My #649 clamp treated EVERY squat/hinge/push/pull exercise as a "main", so a support cyclist's 12-rep accessory (e.g. Leg Press after Back Squat) was wrongly clamped to 6. FIX: only the FIRST exercise of each main pattern is the main; a 2nd same-pattern move is an accessory and keeps its reps. **Route:** gym-split.js clampMainReps. +1 test (16 gym-split green). ✅ built (dev).

**Prescription audit — remaining top gaps (confirm/refine existing items):** #650 pregnancy-privacy scrub (still prompt-only, SAFETY, #1 priority) · #658 gym SELECTION is sport-BLIND (a cyclist & swimmer get IDENTICAL movements, only reps differ — `assignWeeklyGym` takes no sport arg; the #648 lesson un-applied to SELECTION) · #654 triathlete variety covers ONE sport · #652 interval DURATION ignores TTE/W′ (fixed templates) · #657 beginner ramp + cycle ×0.85 modifier + masters gym easing all still prompt-only. World-class = 3.5/5; the 3 to reach 5: sport-adaptive gym selection (#658), push pregnancy-privacy + masters-gym to code (#650), make TTE/cycle metrics SIZE prescriptions (#652/#657).

658. ⬜ **Gym EXERCISE EMPHASIS is sport-BLIND — same movements for a cyclist/runner/swimmer, only reps differ (JM 2026-07-21 "a cyclist should aim for heavy weight but what about core? runners too? swimmers might be something else. bodybuilder compound… do you have that?").** #648/#649 fixed rep/load by focus + #636 guarantees full-pattern BALANCE, but `gym-split.js` assigns the SAME even pattern coverage to everyone; sport-specific movement EMPHASIS is only a one-line prose nudge (coach-engine-strength.md:100 "the limiter in their sport"). A world-class coach differentiates the EXERCISE SELECTION + CORE philosophy per sport:
  - **Cyclist** — heavy BILATERAL legs + posterior chain; core = anti-rotation/anti-extension for power transfer + aero-position endurance; minimal upper (mass = watts/kg penalty).
  - **Runner** — SINGLE-LEG strength (split squat, step-up, SL-RDL) + PLYOMETRIC/reactive (hops/bounds/calf stiffness → economy, Beattie/Blagrove) + hip/glute frontal-plane stability; core anti-rotation.
  - **Swimmer** — UPPER PULL-dominant (pull-ups, lat pulldown, rows) + shoulder HEALTH (rotator-cuff/scap/external rotation — injury-prone) + rotational/streamline core; de-emphasize heavy squat (except sprint/tri).
  - **Bodybuilder** — balanced compound + isolation, all groups ~2×/wk, direct arm/ab volume, near failure.
  - **Core through-line (all endurance):** STABILITY not crunches — anti-rotation / anti-extension / anti-lateral-flexion (Pallof, dead-bug, side plank, carries). Trunk stiffness transfers force + holds position.
  **FIX (world-class, code-decided — the #648 pattern):** a `SPORT_EMPHASIS` map in `gym-split.js` (main-sport → weighted priority patterns · coreStyle · plyo/single-leg/upper bias) that BIASES `assignWeeklyGym`'s pattern + accessory selection by sport (not just frequency), + per-sport guidance in `coach-engine-strength.md` + `create_workout`. Validate across the athlete matrix (skill `validate-athlete-types`). Science: Rønnestad (cyclist), Beattie/Blagrove (runner plyo/economy), swimming S&C (shoulder/pull), Schoenfeld (BB). **Route:** gym-split.js + coach-engine-strength.md + mcp. Ties #648/#649/#636, [[platyplus-gym-engine]].

649. 🧪 **ENFORCE the #648 gym rep scheme in code (the flagship gap — a coach can STILL save a cyclist 3×10 today).** #648 only INJECTED the rep scheme into the prompt; the SAY=DO audit flagged it as the #1 open gap. FIX: `clampMainReps(exercises, focus)` in `server/gym-split.js` (pure, unit-tested) clamps each MAIN compound lift's reps into the focus band (support 3-6 · strength 3-5 · muscle 6-12; accessories/arms/warm-up untouched) → `enforceGymRepScheme(user, plan)` runs in `upsertPlan` (before the teen floor, so ≥5 always wins for under-18s) AND in `reenforceShapeAll` (new gym sweep — stale gym plans with old 3×10 get fixed too; gym had NO sweep before). **Route:** gym-split.js + server.js. +3 tests (15 gym-split green). ✅ built (dev). Completes #648.

650. ⬜ **🔴 P0 SAFETY — PREGNANCY PRIVACY is PROMPT-ONLY → public Strava leak (athlete-type audit, "single most dangerous gap").** The rule "never write pregnancy/trimester in any title/description/public text" lives ONLY in the `# PREGNANCY` prompt block + the `set_activity_text` tool desc — NO code scrubs pregnancy words before a plan title/description syncs to intervals→Strava. An LLM slip = a permanent, PUBLIC medical-privacy leak. FIX: a code scrub (regex `/pregnan|prenatal|antenatal|trimester|expecting|\bbump\b|postpartum/i`) that, when `user.info.pregnant`, strips/rejects those words from plan title/notes/description in `upsertPlan` AND the `set_activity_text` write path (mcp). Highest priority — safety + irreversible. **Route:** server.js upsertPlan + mcp/server.js.

651. ⬜ **%1RM load is DECORATIVE — thrown away (metrics audit, highest-impact metric gap).** The coach picks `pctLow/pctHigh` per focus (#648, e.g. 80-90% for a cyclist), but `create_workout` has NO %1RM param and the UI fills kg from `weightForReps(e1rm, REPS)` — so two lifts at the same reps but different %1RM get the SAME kg; the coach's intensity choice is discarded. FIX: add `pct1rm` to the create_workout exercise schema + persist it; UI/player target = `roundLoad(e1rm * pct1rm/100)` when present, else `weightForReps`. **Route:** mcp/server.js + src/strength.ts (loadForPct helper) + CoachPlanDetail.tsx + GymPlayer.tsx.

652. ⬜ **Interval DURATION is a FIXED TEMPLATE, TTE-blind (per-sport + metrics audits — the direct endurance analogue of the 3×10 bug).** `archetypes.js` ships hard specs ("3×15 or 4×10") identical for a short-TTE athlete and a diesel; `shape-enforce` clamps INTENSITY not DURATION; TTE tailoring is prompt-only ("short TTE ⇒ 3×15-20") and never wired into code. FIX: parameterize the archetype `spec` (or a post-build clamp) to size threshold-rep length off `get_metrics.tteSec` (rep ≤ ~0.6×TTE), + pass W′/D′ MAGNITUDE (not just the binary `bigReserve` flag) into rep sizing. **Route:** archetypes.js + shape-enforce.js + perf-metrics.js.

653. ⬜ **SWIM slips the intensity-ceiling clamp AND the structured-step + variety layers (per-sport + SAY=DO audits).** `create_swim`/`makeSwim` posts no `segments` (only distance/load), so `enforceShape` (which clamps `segments`/`powerStart`) is a NO-OP on swim → a hard CSS/VO2 swim on a pregnancy/teen/maintenance week is only title-relabeled, never intensity-clamped. Also: swim has no `workout_doc` (no device-followable send-off pace, unlike ride/run %ftp/%pace) and CSS can default to nominal 100 if the LLM omits `cssPace100`. FIX: clamp swim `sets[].zone` to a ceiling-derived max zone in shape-enforce; emit structured swim steps + send-off from `cssPace100`; inject the athlete's stored CSS server-side instead of trusting the tool arg. **Route:** shape-enforce.js + mcp/server.js makeSwim + icu-steps.js + server.js.

654. ⬜ **TRIATHLON variety covers only the BIKE; multi-sport DEVELOPMENT PRIORITIES pick ONE sport (per-sport + metrics audits).** `athleteArchetypeBlock` picks a single sport (`cycling ? 'ride' : …`) so a triathlete's run+swim quality days get NO code-assigned archetype (revert to LLM default); and `# DEVELOPMENT PRIORITIES` (server.js ~1945) emits ONE sport's priorities (`cycling ?? running ?? swimming`), dropping a triathlete's run+swim. FIX: run `assignArchetypeBlock` per-sport for triathletes (emit 3 blocks); emit DEVELOPMENT PRIORITIES for ALL the athlete's sports. **Route:** server.js.

655. ⬜ **Gym muscle-BALANCE + EQUIPMENT are injected-not-enforced (SAY=DO audit — re-arms the #636 + #645 bugs).** `validateGymWorkout` checks warm-up/cool-down/unilateral only — NOT that arms/carry/assigned patterns are covered, and NOT that every exercise uses OWNED gear (that's how the band-RDL #645 happened). FIX: extend the gym guard (mcp/gym-guard.js or an upsertPlan save guard) to (a) require the assigned `mustCover` patterns via `patternFromExercise`, (b) reject any gym exercise whose equipment ∉ owned ∪ {Bodyweight}. **Route:** gym-guard.js / upsertPlan.

656. ⬜ **Weekly LOAD BAND is PROMPT-ONLY — no code sums the week's TSS (SAY=DO audit).** The coach can pile TSS past `cap×CTL` (overload) or leave it grey; `plannedTss` exists per plan but is never summed against the band at save. FIX: sum Mon-Sun `plannedTss` in `enforceShapeIntensity`/`reenforceShapeAll`, clamp the newest over-budget session to easy (mirror the existing over-budget path) or 409. Unlocks periodization enforcement too. **Route:** shape-enforce.js + server.js.

657. ⬜ **Adaptation gaps that are prompt-only (athlete-type audit) — beginner intensity ramp (a deconditioned newcomer with a PR goal is code-ALLOWED 2 VO2 days week 1 — gate qualityDays/ceiling on a first-few-weeks/no-load-history RAMP flag, not a fitness bucket, stays within "numbers not categories"); menstrual load modifier ×0.85 never applied to targets + `menstrual` wrongly triggers a quality-day cut (regex bug week-shape.js:92); no masters/postpartum GYM adaptation (recovery spacing / load easing).** **Route:** week-shape.js.

648. 🧪 **Coach prescribed 3×10 leg work to a CYCLIST — rep-scheme wasn't adapted to sport/goal (JM 2026-07-21 "10 rep for leg work for a cyclist, is it really the right thing? bad adaptation to my goals and type of user").** A 5-agent-style audit confirmed: the system decides VOLUME (#534) + BALANCE/VARIETY (#636) in CODE, but the REP/LOAD-INTENSITY dimension was **prompt-only + vague** ("heavy-ish and low-rep", no number) → the LLM defaulted to 3×10 hypertrophy for everyone. Worse, the `# GYM PRESCRIPTION DEPTH` block told it to default to a SLOW-eccentric `3-1-1-0` tempo — the hypertrophy bias, opposite of what a cyclist needs. **FIX (#648, same "code decides, LLM authors within it" pattern):** (1) `server/gym-split.js` — new `REP_SCHEME` per focus (support 3-6 @ 80-90% fast concentric NOT to failure · support_build 4-6 · strength 3-5 · muscle 6-12 · health 8-15) + `resolveGymFocus` (the real 5-way, ported from strength.ts) + `assignWeeklyGym` now returns `repScheme` + `gymBalanceLines` renders it into the MANDATORY `# THIS WEEK'S GYM BALANCE` block; (2) `server/server.js` — use `resolveGymFocus` (not the binary muscle/support regex) so support_build/strength reach the block, and rewrote `# GYM PRESCRIPTION DEPTH` tempo to be focus-aware (explosive `3-0-1-0` for endurance-support/strength/power mains, slow `3-1-1-0` only for hypertrophy); (3) `mcp/server.js create_workout` — reps/tempo/load-by-focus guidance ("cyclist mains HEAVY 3-6, don't default legs to 3×10"); (4) `coach-engine-strength.md` — explicit REP-SCHEME-BY-FOCUS table + the cyclist case with the science (Rønnestad & Mujika 2014, Beattie 2014, Vikmoen 2016); (5) `src/strength.ts` GYM_FOCUS extended with reps/%1RM/intent (single source for Stats). **Also fixed a latent bug:** `inferGymFocus`/`resolveGymFocus` used `\brun\b` which does NOT match "running" → a runner mis-resolved to `health`; now `\brun`. **Route:** gym-split.js + server.js + mcp + coach-engine-strength.md + strength.ts. Tests: src/gym-split.test.ts (+5, 45 green) + strength. ✅ built (dev). ⚠️ mcp change needs host rsync on prod deploy (auto via deploy.sh). Ties [[platyplus-gym-engine]], [[platyplus-coach-architecture]], #534/#636.

**⚠️ PRE-EXISTING RED TESTS (found 2026-07-21, NOT from #648): `src/modules.test.ts` expects "six toggleable disciplines" but `MODULES` now has 7 (swimming/pilates added); `src/stats-hub.test.ts` triathlon-unlock expectation is stale. Unrelated to the coach work — update these expectations (or confirm intent) so the CI build gate is green before promoting.**

647. ⬜ **EXERCISE CATALOG AUDIT — 5 data defects (JM 2026-07-21 "full audit on the exercises"). All fixes land in `scripts/build-catalog.mjs` (regen exercises.json + media re-sync).** Catalog = 4,530 entries (musclewiki/centr/free-exercise-db); `searchExercises` only serves the 3,721 WITH a video; ranking adds `60−name.length` so SHORT names win ties. **Defects, priority order:**
  (a) 🔴 **WRONG DEMO CLIP — 934 rows share a video with a differently-named move** (619 `videoBackfilled` name-matched a MuscleWiki clip; 557 point at a clip used by a *different* movement). E.g. `fedb-Alternating_Deltoid_Raise` (Dumbbell) → a BAND lateral-raise clip; 3 barbell-squat variants → one generic clip; centr push-up family → one shared clip. **Fix:** tighten the video-uplift match (require equipment + full movement, not fuzzy); leave `video` empty rather than graft a wrong one (empty falls out of search); smoke-check backfilled slug shares a token with the name. (centr filenames are opaque hashes → rely on the shared-file signal.)
  (b) 🔴 **NAMING — 214 left/right split pairs + 171 duplicate names (~750 redundant rows).** `…(left)`/`…(right)` should be ONE `eachSide` move; dups like `cool-down`×14, `squat`×7, `cat cow`×5. **Fix:** de-dup pass (keep richest/video row) + fold L/R into one `eachSide:true` entry (needs defect e's hint).
  (c) 🟠 **EQUIPMENT MIS-TAG — 28 loaded moves tagged "Bodyweight"** (→ the #639 weight-lock): `e-25446` Goblet squat + step-out (JM's case), ~12 Sand bag moves, Slam ball squats, `fedb-Farmers_Walk`/`Sled_Push`/`Sandbag_Load`. ROOT: in build-catalog the `EQ_BY_NAME` Bodyweight cue (bare `squat|lunge|raise|plank…`) runs BEFORE the load cues, so any loaded move with a movement word defaults Bodyweight. **Fix:** run `EQ_REASONED`/implement cues FIRST (or exclude names carrying goblet/sandbag/slam/sled/farmer/plate/weighted); map sandbag/slam-ball → Plate/Ball so weight stays editable.
  (d) 🟠 **BANDS — coverage is FINE, the bug is RANKING.** Every pattern has non-band video alts (squat 314 vs band 9, press 447 vs 30…); but short "Band X" names WIN the `60−name.length` tiebreak → `chest press`→Band Chest Press #1, `hip thrust`→Band Hip Thrust #1, top-5 for row/curl/fly. If the coach omits the owned-equipment filter → prescribed to a bandless user (JM's #645). **Fix:** coach/API — always pass owned `equipment`; down-weight `Bands` in `rankExerciseHit` when not owned; regression test: a bandless filter never returns a Bands row.
  (e) 🟡 **NO timed-vs-reps / unilateral HINT in the catalog.** Keys have no `mode`/`timed`/`eachSide`; `seconds` exists on centr-only (1,805) as a generic default on BOTH rep + hold moves → not a signal. So nothing tells the coach a plank/stretch/leg-swing/carry must be TIMED (the #642 warm-up bug) or that a single-leg move is `eachSide`. **Fix:** derive `timed:true` (Mobility/Yoga/Pilates + hold/plank/stretch/carry/wall-sit names) + `eachSide:true` (left/right/single-leg/single-arm/alternating) in build-catalog; coach/plan default `mode`+`eachSide` from these. Feeds (b) + fixes #642/#639-adjacent. **Route:** scripts/build-catalog.mjs (a,b,c,e) + searchExercises/coach (d). Ties #639/#642/#645, [[platyplus-gym-engine]].

645. ⬜ **Plan prescribed BAND exercises JM can't do — must respect the EQUIPMENT profile (JM 2026-07-21 "I don't use bands and it's in my profile, plus bands don't have weight!!").** His windy-day gym had "Band Romanian Deadlift" + "Band Overhead Tricep Extension" — bands aren't in his owned gear (Dumbbell/Barbell/Kettlebell/Bench/Cable/Machine/Pull-up bar/Plate) and can't be loaded in kg. **NOTE: the coach ENGINE already has this rule** — `server.js:1913` injects "Equipment the athlete OWNS: … prescribe ONLY exercises that use this gear … pass equipment= to search_exercises." This instance was a MANUAL build (the windy-day swap) that bypassed the filter — FIXED that plan (kettlebell goblet / barbell RDL / DB bench-row-curl-triceps). **OPEN TODOs:** (a) VERIFY the coach's real auto-built gym sessions honor equipment= (spot-check a coach-built gym for band/unowned gear); (b) AUDIT catalog equipment tags — "Goblet squat + step-out" is tagged **Bodyweight** (→ the #639 weight-lock) when a goblet squat is loaded; pick loaded variants or fix the tag. **Route:** verify coach equipment adherence + catalog tags. Ties #639, [[platyplus-gym-engine]].

644. ⬜ **PWA serves STALE plan data through a hard reload (JM 2026-07-21).** After the server plan was corrected (`GET /api/plan/:id` returns the new sets), the app kept showing the OLD copy (3 sets, 89 min) even after ⟳ + a hard reload — the data layer hydrates from IndexedDB/localStorage and doesn't revalidate a plan it just fetched. "Built ≠ what the user sees." **Fix:** invalidate/refetch a plan on open (or SWR-revalidate); confirm the detail view reflects a server change without a cache wipe. **Route:** client plan cache (Dexie/session) + CoachPlanDetail/plan fetch. Ties [[definition-of-done-validate]].

643. ⬜ **Can't see the FULL PLAN while training — "hard to see the full plan at all time" (JM 2026-07-21).** In the running player you only see the current exercise + dots + up-next; the full list is only in the pre-start preview. Want an at-a-glance overview mid-workout (which exercises, done/remaining, tap to jump). **Proposing a mockup first** (top-bar 📋 toggle → overlay reusing the preview list, current highlighted, done ticked, tap-to-jump). **Route:** GymPlayer.tsx (new overview drawer). Mockup-first per [[show-options-and-mockups-first]].

642. ⬜ **Warm-ups must be TIMED, not sets×reps — ENGINE (JM 2026-07-21).** JM's warm-up showed "3×10 leg swings / cat-cow" — mobility/activation should be a TIMER (~30s), 1 set, no tempo. Fixed JM's plan data manually (mode:timed 30s, plank hold → timed too), but the GYM ENGINE (coach `create_workout` / gym-split) should MODEL warm-up + cooldown mobility as `mode:'timed'`, 1 set, no tempo by default, and holds (plank) as timed. **Route:** mcp `create_workout` guidance + coach-engine-strength.md + gym-split warm-up shaping. Ties [[platyplus-gym-engine]].

641. 🧪 **"Up next" showed the SAME exercise mid-sets (JM 2026-07-21).** Between sets of a reps exercise, "Up next" pointed at that exercise's own next set → read as a bug. Now points at the next DIFFERENT exercise. **Route:** GymPlayer.tsx `upNext`. ✅ built (dev).

640. 🧪 **Can't skip a WHOLE exercise — only sets (JM 2026-07-21 "I cannot skip the whole exercise").** The big ‹‹/›› now jump exercise↔exercise (skip one you're not doing / go back); set-level nav stays on the grid "Skip"/"Log Set" + the rest banner "skip →". (Swipe already did this but wasn't discoverable.) **Route:** GymPlayer.tsx controls → `swipeEx`. ✅ built (dev).

639. 🧪 **"bodyweight" LOCKED the weight field on a loaded move — can't track weight (JM 2026-07-21).** Goblet Squat (clearly dumbbell in the demo) rendered a static "bodyweight" label with no weight input, because the catalog entry is equipment-tagged bodyweight. A "bodyweight" move is often loaded (goblet squat, weighted vest, DB). Now ALWAYS shows the weight input with a "BW" placeholder → you can log the actual weight. (Also: audit catalog equipment tags — goblet squat shouldn't be tagged bodyweight.) **Route:** GymPlayer.tsx set grid (`gBW` → placeholder only). ✅ built (dev).

668. 🔨 **[built — QA] Player CTA can start BELOW THE FOLD on a tall session (found in QA browser walk 2026-07-21).** FIX: `.gp2-gridcta` is now `position:sticky; bottom:0` inside the scrollable `gp2-gridwrap` — opaque `--bg`, a `--line` hairline + upward fade so it reads as a floating action bar pinned to the bottom of the scroll. Skip + Log-set/Finish are ALWAYS on-screen regardless of grid height. **Route done:** styles.css `.gp2-gridcta` (CSS-only, no markup change). Verify in Chrome on a 5+ exercise session.
668-OLD. ⬜ **Player CTA can start BELOW THE FOLD on a tall session (found in QA browser walk 2026-07-21).** #638 fixed the overlap (the green "Log set → rest" now sits cleanly above the dots), but on a session with a full set grid the primary CTA is inside the scrollable grid area and can require a scroll to reach — the primary action should ALWAYS be visible. Fix: pin the gp2-gridcta (Skip + Log set/Finish) as a sticky footer of the grid area (or move it out of the scroll), so it's on-screen without scrolling. **Route:** GymPlayer.tsx gp2-gridcta + styles.css. Ties #638/#662.

638. 🧪 **Player bottom OVERLAPPING — green "Log Set" CTA collides with the pagination dots (JM 2026-07-21).** Two `flex:1` siblings (video stage + set grid) fought for height, pushing the in-scroll green CTA on top of the dots/up-next/controls. The demo stage is now capped (`max-height:32vh`) when the set grid shows, so the grid + CTA + dots + up-next + controls all fit. **Route:** styles.css `.gp2-stage--tight` + GymPlayer.tsx class. ✅ built (dev). *(Watch: verify on a small screen the grid still fits; may need to also thin the bottom cluster.)*

635. ⬜ **GYM variety imperfect — same lifts every session, muscle groups unbalanced, NO ARM work (JM + wife 2026-07-21).** Xenia noticed the same Shoulder Press + Romanian Deadlift appear in EVERY session, and there are no arm exercises. **The tension:** `coach-engine-strength.md` VARIETY (#573) deliberately KEEPS the 1-3 main compound lifts STABLE week-to-week (so load/reps progress) and rotates ACCESSORIES — so repeating the mains is BY DESIGN (progression). BUT two real gaps: (1) accessory rotation may not be working (same accessories recurring), and (2) NO explicit MUSCLE-GROUP / movement-pattern BALANCE — over a week the plan should cover push · pull · squat · hinge · core · **arms** · (carry), and right now arms are neglected. **JM's questions to answer in the audit:** what ARE the variety criteria? do we balance per muscle group? **Likely fix:** add a weekly muscle-group/movement-pattern balance rule to the gym engine (+ maybe a code check like the endurance archetype assignment — assign the week's split so all groups incl. arms are hit, mains stable + accessories rotated), and verify the accessory-rotation look-back (#573) actually fires. **Route:** coach-engine-strength.md + possibly a gym-variety code layer (peer to archetypes.js). Ties [[platyplus-gym-engine]], the #620 code-driven-variety pattern. **AUDIT queued (JM: "audit this when you have the chance").**

**★ 2026-07-21 COACH REWORK → WORLD-CLASS (JM's session: "world-class, data-driven coach that reads the science + applies it to all athlete types, all sports, all variables"). Shipped to PROD #624-631 + a 5-agent audit. All ✅ built+prod; pending JM browser sign-off.**

631. ✅ **Postpartum ramp + teen 1-RM gym enforcement + show-progress (JM 2026-07-21).** POSTPARTUM: `weekShape` graded return (0-6wk maintenance/0 quality/pelvic-floor; 6-12wk 1 quality; ≥12 normal) — no snap to a full build after birth; Profile "recently gave birth?" date field (`info.postpartumSince`). TEEN 1-RM: `enforceTeenGym` floors under-18 near-maximal (<5-rep) gym sets on save (endurance was enforced, gym now too). SHOW PROGRESS: the review states the measured gain ("threshold up ~8W in 6 weeks") from get_metrics. **Route:** week-shape.js + server.js + Profile.tsx. Tests green.

630. ✅ **Periodization autoregulation + season macro + novice ramp-in + strength-for-endurance (JM 2026-07-21).** `periodizationPhase` takes Form → deep-negative Form forces an UNPLANNED recovery week (not just the calendar); build weeks now actually RAMP (were flat); season MACRO phase (base/build/peak by weeks-to-race); NOVICE ramp-in prompt (no code bucket — reads the profile, per JM's "profile+coach is enough"); strength-for-endurance added to run+ride engines (Rønnestad/Beattie economy). **Route:** periodization.js + server.js + engines.

629. ✅ **Coach audit quick-wins (JM 2026-07-21).** From the 5-agent audit: triathlon metrics gate bug (bike/run were excluded for a ['triathlon'] athlete); flat build-week targets (real ramp now); modern fuelling numbers (run 30-60→60-90 g/h + cycling/tri targets); swim `athleteProfile` branch (swimmers' # DEVELOPMENT PRIORITIES was empty); stale-comment + get_metrics swim-docstring cleanup. **Route:** server.js + perf-metrics.js + mcp + engines.

**★ COACH AUDIT (5 agents, 2026-07-21) — scored ~3.6/5 "very good, not yet world-class". Roadmap: all shipped via #629-631. Remaining depth parked: full macro-modulation of weekShape by season phase, an athlete-facing progress CARD (currently review-surfaced only), distance-aware taper length, creatine/VBT engine adds, hand the coach the app's computed readiness 1-5 verbatim. See memory [[platyplus-coach-architecture]].**

628. ✅ **A-race date → periodized taper (JM 2026-07-21).** GoalsPicker captures a race date + name (when "Race/event" focus) → `info.raceDate`; `periodization.js` drives a 2-week taper + the season macro phase off it. **Route:** GoalsPicker.tsx + periodization.js.

627. ✅ **PROGRESS CHECK — measure progress toward the goal (JM 2026-07-21).** `# PROGRESS CHECK` prompt block: coach compares get_metrics now-vs-then (threshold/TTE/EF/CTL + CS·D′/CSS), changes the stimulus on a stall, emphasized on the recovery week. **Route:** buildSystemPrompt.

626. ✅ **Multi-week PERIODIZATION (JM 2026-07-21 "coach must progress, not flat").** `server/periodization.js` — build→build→peak→recovery meso, CTL-scaled, age-aware, sport-agnostic (CTL/TSS aggregate all sports); `# THIS BLOCK'S PROGRESSION` block; load budget points at the periodized target. **Route:** periodization.js + buildSystemPrompt. Unit-tested.

625. ✅ **Coach review "Next" = the OBJECTIVE, not the next workouts (JM 2026-07-21).** ⟵ *this is the "description with next activity mentioned which is wrong" JM flagged.* His real Jul 20 review listed "Today: Z2 spin… Tomorrow: Threshold 4×10… Protect sleep" — naming upcoming sessions + recovery = WRONG. The review's `next` now talks the OBJECTIVE the athlete is building toward + how this session moves it, NOT the calendar, no recovery. Reverses #580. **Route:** reviewMsg (server.js) + save_coach_review `next` (mcp).

624. ✅ **Plan build reads get_metrics + prescribes from the FULL profile (JM 2026-07-21 "is the coach using ALL metrics for ALL sports?").** The daily plan-build now reads get_metrics (it read wellness/checkins/history but never the perf profile) and prescribes off TTE (interval duration), W′/D′ (rep length), EF (base) — cycling/running/swimming. **Route:** dailyAdaptMsg. Also #622(c) done: `# DEVELOPMENT PRIORITIES` wires the Stats focus into the plan.

623. 🧪 **Planned-workout DESCRIPTION = objective + success criteria + fuelling ONLY; the coach authors every cue and they must be CONSISTENT / non-contradictory (JM 2026-07-21).** JM: a planned session's description should carry the training OBJECTIVE + SUCCESS CRITERIA + FUELLING cues — and NOTHING else: NO recovery, NO Eat/meal/supplement section, NO mind, NO mention of past or future sessions (just the session in front of them). Critically: "the coach defines ALL the cues, so any cue should be consistent and not in contradiction" — one authoritative cue format, applied across every layer. **Built (all layers, #623):** (1) PROMPT — `dailyAdaptMsg` WORKOUT DESCRIPTION block rewritten to the 3-part shape (objective / success criteria / fuelling), explicitly no recovery/eat/mind/past-future, "you are the single author of every cue, keep them consistent". (2) MCP tools (`mcp/server.js`) — create_ride/run/swim/workout coaching-shell guidance aligned (objective + success + fuelling cue, "do NOT set recovery or mind"); removed the contradicting "recovery rides on the workout" comment; also deferred the swim tool's hard-coded weekly CSS-set count to the WEEK SHAPE block. (3) UI (`CoachPlanDetail.tsx`) — dropped the 🛌 Recovery + 🧠 Mind + 🍽️ Eat(meals/supplements) sections; the detail now shows Objective + Cues + Success + ⛽ Fuel(cue) only (recovery/mind fields still in the model, just not rendered). Reverses the earlier "recovery rides on the workout text" (#JM 2026-07-15). Ties [[platyplus-coach-public-text-voice]] (distinct: that's the PUBLIC completed-activity text), #600 (current session only, no invented surrounding-day context). 🧪 **TEST:** re-adapt on QA/prod → a planned workout's detail shows only objective + success + fuel, no recovery/eat/mind, no past/future talk; MCP synced to host on deploy. **Route:** dailyAdaptMsg + mcp/server.js + CoachPlanDetail.tsx. ✅ built (dev, #623).

622. ⬜ **Stats "🎯 What your coach will work on" is DISCONNECTED from the actual plan (JM 2026-07-21).** The Benchmarks/Stats card (`src/Benchmarks.tsx:572` → `prof.focus` from `src/athlete-profile.ts:68-80`) computes a recommendation purely from the athlete's BENCHMARK PROFILE (rider type / TTE / W′ reserve) — e.g. a long-TTE "diesel" always gets "Raise the ceiling — VO₂ 4-5×3-4 min 1-2/wk · keep the punch (weekly sprint) · aerobic volume." It NEVER reads the scheduled plan, so the heading "What your coach WILL work on" PROMISES coach work the plan may not contain → SAY=DO mismatch (JM saw the VO₂/sprint focus in Stats but not in his plan). **Fix options (JM to pick):** (a) RE-LABEL so it's honestly a recommendation not a plan promise ("Training that would move your numbers most" / "Focus for your profile") — quick + honest; (b) DERIVE the text from the real plan (read the scheduled quality sessions, summarize) — accurate but couples Stats to the calendar; (c) FEED this profile-focus INTO the coach's plan build so the plan actually contains it (SAY=DO-correct — the build cyclist's shape already allows 2 quality incl. VO₂/threshold via weekShape/archetypes, so wire the profile focus into `dailyAdaptMsg` and the promise comes true). **Recommend (a) now + (c) as the real fix.** Ties [[definition-of-done-validate]] (SAY=DO), the #620 shape/variety work. **Route:** src/athlete-profile.ts + Benchmarks.tsx (label) and/or dailyAdaptMsg (wire focus in).

620. 🧪 **CODE-DRIVEN variety + the "my wife has just tempo runs" fix (JM 2026-07-20).** Continuation of #599 (variety) + #613/#615-618 (weekShape dose + enforcement). **Two things:** (1) **Root-cause of the pregnant-runner "all tempo":** `enforceShapeIntensity` was ASYMMETRIC — `isModerate()` (counts OTHER sessions) matched a title-only "Tempo Run" via `/\btempo\b/`, but `thisIsModerate` did NOT → a tempo run consumed the week's moderate budget when counted against others, yet was never clamped itself, so every excess tempo survived. Now symmetric (`thisIsModerate = isModerate(plan)`) + an over-budget title-only tempo is relabeled to a varied easy cue. NB her PROD plan was actually already dosed right (1 tempo + easy/wk); the complaint was MONOTONY (same "Tempo Run"/"Easy Aerobic Run" titles every week) + this latent bug. (2) **Code-driven variety (`server/archetypes.js`):** `weekShape` sets the DOSE (count + ceiling), archetypes set the FLAVOR — `assignArchetypeBlock` rotates the full quality range (skipping recent, respecting the ceiling) + gives each easy day a distinct cue, emitted as a `# THIS BLOCK'S VARIETY` prompt block right after `# THIS WEEK'S SHAPE`. Don't hope the LLM varies — assign it (same principle as the enforcement). Also: `enforceShapeIntensity` extracted to a PURE, unit-tested `server/shape-enforce.js`; `honestTitle` downgrades any clamped title to its true level + rotates easy relabels (no more flat "Easy Aerobic Run"); `reenforceShapeAll` re-pushes clamped sessions to intervals (prod-only, so Garmin matches) and now runs in its OWN try (ALWAYS fires, even if an LLM pass hiccups). **VERIFIED on QA:** the variety block is live in Xenia's real coach system prompt — for her pregnant/tempo-ceiling profile it assigned **Strides** (submaximal, pregnancy-appropriate — not sweet-spot) + distinct easy cues (conversational · trail), correctly capped at 1 moderate. **Route:** coach plan builder (`server/{archetypes,shape-enforce,week-shape,server}.js`). Ties [[platyplus-gym-engine]] (#573 look-back pattern), the `validate-athlete-types` skill. 🧪 **TEST:** (a) `npm test` — `src/{archetypes,shape-enforce,week-shape}.test.ts` (34 tests: exact Xenia scenario 2 sweet-spots + tempo → ≤1 moderate; 3 title-only tempos → ≤1; persona matrix pregnant-T3/build/teen/swimmer; small-pool weekly alternation); (b) on QA/prod after a daily-adapt, confirm a pregnant runner's plan is mostly easy + ≤1 moderate with VARIED titles (strides/tempo, not all "Tempo Run") + varied easy cues, and a build athlete rotates ride archetypes. ✅ built (dev, #620); QA end-to-end adapt in progress; **pending prod promote + JM browser sign-off.**

608. 🧪 **ROOT CAUSE of Xenia's missing plan: the coach adapt TIMED OUT mid-build (240s too short for a strength rebuild) (JM 2026-07-20).** Confirmed from the coach's OWN session transcript (`~jmf/.claude/projects/-home-jmf-platyplus-chat/*.jsonl`): her daily-adapt reads wellness/checkins fine (connected:true), sees an EMPTY schedule, then makes **25-40 tool round-trips** building 2 gym sessions — ONE `search_exercises` per exercise + warmups/cooldowns + runs — and gets SIGKILLed at the chat-helper's **240s** timeout BEFORE any `create_workout` fires → zero sessions saved. A CYCLING adapt (structured ride segments, ~no searches) finishes fast, so it looked user-specific. NOT auth (JM's 1-yr `CLAUDE_CODE_OAUTH_TOKEN` in `/home/jmf/.claude-coach-token` is valid + loaded by the service — verified), NOT intervals, NOT contention. FIX: raise the coach timeout **240s→600s** (`chat-helper/server.mjs` + gymapp `runCoachTask` fallback); deploy.sh auto-rsyncs+restarts the helper. **Follow-up (#590 token-thrift):** kill the per-exercise `search_exercises` round-trips — batch or a strength template — so it's fast AND cheap, not just un-timed-out. **Verify:** trigger Xenia's adapt → it completes + her 14-day plan populates. **Route:** coach timeout + (later) strength-build efficiency. ✅ fix built (dev).

606. 🔨 **Coach 14-day planner must NOT require intervals — Xenia's plan died because cleanup ran but rebuild was gated off (JM 2026-07-20).** ROOT-CAUSE (from her audit log): today `handle-missed` (client-triggered, NOT gated) had the coach REMOVE 7 of her sessions, but `runDailyAdapt` — which rebuilds the 14-day horizon — was gated behind `!user.icuKey` (server.js:3493), so nothing refilled → empty plan since Jul 17. FIX (part 1, done): drop the `icuKey` gate — gate on `coachProfile` only; the planner is intervals-OPTIONAL (coded skeleton — verified VALID for her config: 8 training days run+gym; readiness degrades to check-in scores; pushPlanToIcu no-ops without an athlete). ⚠️ OPEN (part 2): her audit shows `runDailyAdapt` ALSO ran today (extras=today, likely via the un-gated MANUAL endpoint) yet created ZERO forward sessions — so the coach's CREATE step is silently producing nothing even when it runs. Skeleton is valid, so this is in the coach task itself (reads 409-ing without an athlete? coach giving up?). NEEDS a live prod adapt with verbose logging to pin the create failure. **Route:** coach planning gate (server.js dailyAdaptTick) + adapt create step. ✅ part 1 built (dev); part 2 open (prod trace).

605. ⬜ **Coach review OVER-PRAISES trivial/test sessions — called a ~2-min, 2-set session "solid" (JM 2026-07-20).** The activity coach review isn't calibrated to actual effort/volume/duration: a token session (a couple of sets, a few minutes) got a glowing "solid session". Praise must scale to what was actually done — a tiny/partial/test session should be recognized as such (light/opener/test), never inflated. Ties [[platyplus-coach-public-text-voice]] + #533 (reflective, honest — not a cheerleader) + #600 (don't invent context). **Fix:** review-voice rule in `coach-engine*.md` + the review trigger guidance — gate superlatives on real thresholds (duration/sets/load vs the athlete's norm); for a trivially small session, say so plainly. **Route:** coach review voice. (This instance was a browser-TEST gym session — delete that test review/log from QA.)

604. 🧪 **Athlete number gone from Settings + Xenia's coaching dead since Jul 17-18 — both from #582 (JM 2026-07-20).** #582 (`2205cd4`, Jul 18) removed the "Athlete id" field from Settings and made the athlete resolve SERVER-SIDE from each user's OWN key (`/athlete/0`). Side effects: (a) the athlete number vanished from Settings (no display at all); (b) any user WITHOUT a personal key (Xenia — she was on the shared/seed athlete) can no longer resolve an athlete → her `icuAthlete` went blank → coach sync + 14-day auto-plan broke (her plans frozen at Jul 17). FIX (part 1, done): restore a READ-ONLY athlete display in Settings (server-resolved, no client-set so the i28814 leak #453 stays fixed) — shows "Athlete <id>" when connected, or red "re-save your key" when a key resolves none. **Part 2 (Xenia, TODO):** decide — her own intervals key (server resolves her athlete) OR decouple coach planning from intervals so a check-in-only user still gets a plan. **Route:** Settings connections + coach planning gate. ✅ part 1 built (dev).

603. ⬜ **⚠️ `sync-coach-engine.mjs` is DRIFTED + DESTRUCTIVE — running it REVERTS direct edits to the shipped engines (found 2026-07-20 while cleaning cycling).** The `server/coach-engine-*.md` are labelled "GENERATED … do not edit here," but they've been HAND-EDITED directly for a while (#479 indoor/outdoor guidance, #614 variety, public-text changes) WITHOUT back-porting to `coach-engine-src/`. Proof: a fresh `node scripts/sync-coach-engine.mjs` (a) DELETED the #479 cycling guidance (not in source), and (b) GREW `coach-engine.md` +2900 words by re-adding source sections (nutrition/psychology/weekly-checkin) the committed file lacks — so the committed generic engine is ALSO out of sync. The "sync is lossless" (#516d) is no longer true; **running the sync now silently loses work.** Nothing auto-runs it (not in `npm build`), so it's latent. **Fix (a reconciliation task):** back-port every direct edit since the last clean sync INTO `coach-engine-src/`, decide whether the missing generic sections belong, then make the sync lossless again (or retire the sync + treat the shipped `.md` as canonical hand-maintained). Until then: **edit the shipped `server/coach-engine-*.md` directly; do NOT run the sync** (flagged in the cycling file's top comment). **Route:** coach-engine build tooling. **Meanwhile — CYCLING ENGINE CLEANED (JM "clean all you need", token-thrift #590), applied DIRECTLY to the shipped file:** removed the 6 `<!-- source -->` provenance comments; fixed 4 DEAD external links (`/Users/jmfiset/dev/cyclingcoach/…` ×4) that told the coach to "use templates in" a file it can't read → self-contained truth (the `save_coach_review` tool IS the format); tightened the shared HEADER + **fixed a contradiction** (header said public TITLE "human+creative"/DESCRIPTION "scientific" but `set_activity_text` mandates PLAIN both → aligned to the tool); de-duped the weather-naming rule (already in `coachIdentity`). No coaching-method change. Deep prose-trim deferred until this drift is reconciled (a reword now risks parity AND being reverted). ✅ **RESOLVED via the RETIRE path (2026-07-20):** the shipped `server/coach-engine-*.md` are now declared **CANONICAL, hand-maintained** — (1) `sync-coach-engine.mjs` **refuses to run** unless `COACH_ENGINE_SYNC_FORCE=1`, with a loud drift warning (no more silent revert); (2) all 3 engine headers now say "hand-maintained — edit directly" (was "GENERATED — do not edit"); (3) CLAUDE.md's coach-engine row updated to "edit the shipped files directly; sync deprecated." `coach-engine-src/` is kept as historical reference only. **One CONTENT question for JM (separate):** the generic `coach-engine.md` is MISSING `instructions_nutrition_recovery` + `instructions_sports_psychology` + `weekly_checkin` (they're in the stale source). This is ALMOST CERTAINLY intentional — Eat + Mind are deactivated (#491, coach gated), and the weekly-checkin has its own handling — so the coach correctly doesn't carry them. But confirm: should the coach still have TRAINING-fuel guidance (fueling around hard sessions, not meal-planning) and mental cues, or is it right to omit them while Eat/Mind are off? If they should return, that's a small content add, NOT a re-sync.

602. 🧪 **Gym recap insight was unsafe / low-value — "Recover well — protein tonight" (JM 2026-07-20).** JM: generic nutrition advice is risky without the athlete's metabolism + goals ("30g of protein with x g of carbs? how will you know if you don't know the user's metabolism — be careful with nutrition advice"). FIX (`GymSummary.tsx`): the canned insight is now FACTUAL only — "N sets · N reps · N PR 🏅 logged. Your coach reviews it above." — NO nutrition/recovery prescription baked into a hardcoded client string. Personalised recovery belongs to the COACH review shown above, which HAS the athlete's context (CTL/ATL, goals, cycle) — a canned string can't. **Open Q for JM:** want an explicit-but-SAFE recovery cue in the line (e.g. "give this muscle group ~a day before loading it hard again" — training, not nutrition), or keep it factual + defer to the coach? **Route:** gym summary insight. ✅ built (dev) — needs JM's call on wording before prod.

601. 🧪 **Remove VOLUME from the gym recap — not actionable for a user or the coach (JM 2026-07-20).** Tonnage (`totVol`, "384 kg moved") dropped from `GymSummary.tsx`: the hero tile (Volume → Exercises count), the insight line, the per-exercise "— N kg" suffix, and the per-exercise volume BAR (`gymbar`). Sets · Reps · Time + est 1RM + PRs stay (those ARE actionable). **Route:** gym summary. ✅ built (dev); browser-verify then promote. (Also: the "BW"→"bodyweight" fix for glute bridge = commit a986e51, already on QA, still PENDING PROD PROMOTE — that's why BW still shows on prod.)

600. 🔨 **Coach review invents surrounding CONTEXT — "Easy Z2 the day after gym" when there was no gym (JM 2026-07-20).** On an activity's coach review, the coach framed the session by an assumed prior day ("the day after gym") that didn't happen — JM did no gym the day before. The review must **focus on THE ACTIVITY itself** (what was done, execution, feel, what it means) and NOT assert surrounding-day context unless it's VERIFIED from real data (`get_recent_activities` / the calendar). If the coach wants to reference yesterday/tomorrow, it must CHECK first (like the #581/#5019 relative-day labels) — never assume "the day after X." **Fix:** coach-engine review-voice rule (`coach-engine*.md` + the review trigger / `save_coach_review` guidance): review the activity on its own terms; only mention adjacent sessions when confirmed. Ties [[platyplus-coach-public-text-voice]] + #533 (reflective, not prescriptive) + #425. **Route:** coach review voice. **PINNED the real examples (browser, QA jmf, 2026-07-20):** 7 of 26 coach reviews assert adjacent-day context — e.g. `i167548333` "low-load frequency day after Saturday's 105 TSS long ride" + "Back-to-back easy days Mon/Sun after Saturday's long sweet-spot effort"; `rev-2026-06-23` "sat this easy the day before the big Saint-Jean SS ride". Some may be real, but the coach was ASSUMING the sequence, not verifying it. **🔨 BUILT (2026-07-20):** new shared `REVIEW_OWN_TERMS` const (`server/server.js` ~1102) wired into ALL 3 review triggers (`triggerActivityReview`, `triggerPlanReview`, daily `reviewMsg`) + the `save_coach_review` MCP tool (`mcp/server.js`, rsynced to host) + the coach-engine workout-analysis do-not-include list (`coach-engine-cycling.md`): "only reference an adjacent session when VERIFIED in get_recent_activities/the calendar — never assume it." Extends the existing SAY=DO discipline (future "next") to the review's PAST/framing. 🧪 **TEST:** on QA, trigger a coach review on an activity that has NO gym/adjacent session the day before → the review must describe the session on its own terms and NOT assert "day after gym" / "coming off X" unless that session is actually on record. (Prompt-rule change, not unit-testable — behavioral check.)

599. 🔨 **Workouts lack DIVERSITY — vary cycling/running/swimming while respecting objectives ("boring is dangerous") (JM 2026-07-20).** JM's rides feel samey. Want the coach to inject VARIETY across all endurance sports (ride/run/swim) — different session TYPES, terrain/route, structure, cues — WITHOUT drifting from the athlete's objective + the weekly load/periodization. Ties the gym-variety work #573 (which made gym look-back before repeating warm-ups/accessories — extend that "look back, don't repeat" idea to endurance). **Design questions (options-first before building):** (a) is variety a COACH-planning behavior (the daily-adapt/plan builder picks varied session archetypes — endurance/tempo/VO2/sweet-spot/over-unders/fartlek/hills/technique — cycling through them per the training goal + phase) or an app nudge? (b) how much variety vs consistency (some blocks NEED repetition for adaptation — respect that); (c) per-sport archetype libraries (bike vs run vs swim) mapped to zones/goals. Likely a `coach-engine-*.md` planning-variety rule + a "recent session archetypes" look-back injected into the plan builder (like #573 for gym), so the coach doesn't serve the same workout shape repeatedly. **Route:** coach engine / plan builder (cycling/running/swim). Mock/plan the archetype-rotation approach first. **🔨 LARGELY BUILT — the worker's #614 already did the bulk (audited 2026-07-20).** Every endurance engine now has a full **VARIETY — rotate archetypes, look back** section (`coach-engine-cycling.md` ~652, `-running.md` ~51, `-swimming.md` ~37): per-sport ARCHETYPE ROTATION menus (cycling: easy Z2/long/tempo/sweet-spot/over-unders/threshold/VO2/torque-hill/fartlek/recovery · running: E/long/recovery/strides/tempo/M-pace/I/R/hills/fartlek/progression · swim: endurance/CSS/broken-threshold/VO2/descending-pyramid/speed + drill+toy rotation), terrain/route + cue variation even on easy days, the **~10-day no-repeat RULE with a block-repetition caveat** (some blocks NEED the same key session — vary the wrapper), all WITHIN the week's intensity ceiling + objective (respects periodization). Mechanism: **`get_session_history` MCP tool** (#614, recent+upcoming plans, all sports) + `dailyAdaptMsg` "VARY IT — call get_session_history first, rotate per your sport engine's VARIETY list". Modeled on the gym #573 pattern. **My delta (#599, 2026-07-20):** unified the 3 engines' LOOK BACK FIRST step onto the purpose-built `get_session_history` (was list_schedule + get_recent_activities = 2 calls) so prompt+engines reference the SAME cheap look-back (consistency + token-thrift). **Deliberately NOT done:** a deterministic server-side anti-repeat guard — it would fight the legitimate block-repetition the engines allow (same mistake as the #610 skeleton that killed individualization); variety stays coach-driven. 🧪 **TEST:** on QA over several daily-adapt passes, confirm the coach rotates ride/run archetypes (not "Easy Aerobic Spin" every time) while keeping key block sessions + the intensity ceiling. Depends on #614 landing.
...
479. 🧪 **Target RANGES must be correct — show a range (not a flat value), and read smallest-first.** Two facets, combined
    ...
    **Route:** bug (worker owns (a); (b) done client-side).
    ...
    intervals). ✅ **REWORKED into the full INDOOR/OUTDOOR model (JM 2026-07-20, after researching TrainerRoad/intervals/Garmin — JM: "other platforms do it easily").** intervals' own model = a per-workout indoor flag (indoor → single specific watt/ERG; outdoor → power range/band). Built: **`plan.indoor`** drives `ev.type` (**VirtualRide**=indoor vs **Ride**=outdoor) + the encoding (`dispSegs = outdoor ? bandSteadyPower(segs) : segs`), and round-trips (`icuEventToPlan` maps VirtualRide→ride+indoor). **Coach:** MCP `create_ride` gains an `indoor` param + updated desc + `coach-engine-cycling.md` guidance (infer trainer→indoor/specific, endurance→outdoor/range). **USER OVERRIDE (JM's "let the user decide after"):** `POST /auth/plan/:id/indoor` + `authApi.setPlanIndoor` + an **Indoor/Outdoor toggle on the ride detail** (`CoachPlanDetail`, theme tokens) — flip anytime → re-push re-encodes (Platyplus owns the plan). Default (unset) = outdoor(band), fixing JM's original "flat 171 W, no range" on the Garmin. tsc+build green. ⚠️ MCP change syncs to the host on prod deploy. **Test:** open a planned ride → Indoor/Outdoor toggle; Outdoor pushes a band, Indoor a specific target; flip → the intervals event re-encodes. **Route:** intervals encoder + coach + ride-detail UI.
...
740. 🔨 [built] **The "💤 Rest day" marker was too subtle (JM 2026-07-22).** Replaced the small meta text with a prominent cyan REST-DAY BANNER ("Rest day · Recovery is training too — your coach is keeping this day clear") on the day view. Route: src/pages/Today.tsx + styles.css.
741. ⬜ **Phone push not offered for the new notification types (JM 2026-07-22, "why not phone?").** New releases / New activity / Achievements & PRs / Missed session show "—" for Phone because there's no server-side push SENDER for them yet — the app only pushes coach reviews / plan-changes / reminders. Enabling phone needs a per-type sender (fires pushNotification when the event occurs) — the same detector work as Achievements/Missed. Route: src/push.ts PushPrefs + server push-send + the detectors.
742. 🔨 [built] **Notifications Settings gear opened the page but not the exact section (JM 2026-07-22).** The gear links to /settings#ob-notifications but the section only opened on the onboarding path. Now any #ob-… hash opens + smooth-scrolls to its section; the Notifications collapsible got id="ob-notifications". Route: src/pages/Settings.tsx.

### 🔍 WHOLE-APP AUDIT #2 (2026-07-22) — 65/100, top gaps (from the 33-agent scored audit; mockups/whole-app-audit.html)
743. ⬜ **[audit2·critical] Teen safety silently OFF without a DOB — falls through to adult 2×VO2 build + near-1RM gym** (Teen runner (performance)) — ageYears derives from optional user.info.dob; when unset teen=false and the athlete gets the default build band (qualityDays=2, ceiling vo2=130%) plus adult gym loading via the same gate. Code-enforced but conditioned on optional self-reported data with the UNSAFE default — the opposite of safe-by-construction for the persona most exposed to harm. Route: server/week-shape.js:82 (gym: server/server.js:1848).
744. ⬜ **[audit2·high] Empty-goal beginner defaulted into a full BUILD (2 quality days, VO2 ceiling)** (Beginner · general fitness · no benchmarks) — loadBand is derived purely from free-text goal words; an empty goal.focus/notes falls to the else branch loadBand='build', qualityDays=2, ceiling='vo2'. A vo2 ceiling of 130% means the shape clamp does nothing, so a raw beginner who never typed a goal is prescribed two VO2-max quality days with zero ceiling protection. Route: server/week-shape.js:75-89.
745. ⬜ **[audit2·high] Early-postpartum impact contraindication is prompt-only — a run is never converted to non-impact** (Postpartum runner) — weekShape returns impact:'none' for <6wk postpartum but enforceShape only clamps power-% and relabels titles — it never changes modality, and the code admits the conversion is 'Coached via the prompt.' A <6wk-postpartum runner keeps an easy RUN (ground-reaction load on a healing pelvic floor) with nothing at save or in the sweep preventing it. Route: server/shape-enforce.js:81 (week-shape.js:54-57).
746. ⬜ **[audit2·high] No RED-S / energy-availability safeguard for a performance teen runner** (Teen runner (performance)) — Adolescent distance runners are the canonical RED-S / bone-stress population, yet there is no code OR prompt screen for underfueling, weight-loss-as-goal, or growth caution — coach-engine-running.md has no teen/energy section; the only signal is one generic clause in dailyAdaptMsg. Effectively absent for the highest-harm persona. Route: server/coach-engine-running.md (server/server.js:3935).
747. ⬜ **[audit2·high] Concurrent-training (strength↔quality-endurance) separation is detection/prompt-only, never enforced** (JM (build) + Endurance-athlete-who-wants-muscle) — concurrentGymCollisions only DETECTS; the save site console.logs the collision; the sole prevention is injected prose. A heavy leg day can land the day before a quality ride and nothing at save moves or rejects it — the codebase's own repeated lesson is 'the prompt was ignored.' A structural quality rule left prompt-only is a defect by the bar. Route: server/shape-enforce.js:34 (server/server.js:2100).
748. ⬜ **[audit2·high] Unknown-trimester pregnancy defaults to T1 (least conservative), skipping the supine-gym clamp** (Pregnant runner (no date entered)) — pregnancyStage().trimester=null → every consumer defaults to 1 (tempo ceiling + a moderate day allowed), and the supine/vena-cava gym clamp only fires at trimester>=2. An athlete actually in T2/T3 who hasn't entered a date gets the least-conservative envelope. Safe-by-construction demands the unknown case default to the MOST conservative (T3). Route: server/week-shape.js:65 (cycle.js:110, server.js:1837,1844).
749. ⬜ **[audit2·high] No-benchmark athlete gets %-of-threshold targets that cannot resolve; RPE fallback is prompt/onboarding-only** (Beginner · no benchmarks) — Sessions encode {power:%ftp}/{pace:%pace}; nothing in save/enforce detects a missing FTP/threshold and converts to an RPE/talk-test anchor. The only handling is onboarding prose (which itself says the workout is 'useless' with no data) — absent on the daily-adapt build path, so pushed % targets don't resolve on the watch. Route: server/server.js:2229-2232.
750. ⬜ **[audit2·high] Weekly hypertrophy VOLUME (10-20 sets/muscle/wk) is prompt-only, never code-enforced** (Bodybuilder / strength) — The engine's own defined crux is weekly sets/muscle, but set_gym_target only STORES a band; at save the only gym guards are reps, structure-dedup, pregnancy + teen clamps — none checks weekly set volume. A bodybuilder can be handed a chronically under-dosed week with no code correction, and a cold-start user reverse-derives to full-body 1×/wk from cadence. Route: server/server.js:3517 (frequency: server.js:2019).
751. ⬜ **[audit2·high] Swim weekly quality-day COUNT is prompt-only, and over-budget swim only rewrites the title** (Swimmer (performance)) — enforceSwim clamps each set's zone but never counts hard swim days; the #620 over-budget logic reads plan.segments/titles, not plan.swimSets, so a performance swimmer can be saved 4+ race-pace sessions/wk uncapped. When it does fire, the title becomes 'Easy Swim' while the sets stay '8×100 @ Z4' — cosmetic, not enforced. CSS-titled swims also evade the moderate budget entirely. Route: server/swim.js:18 (shape-enforce.js:30,101).
752. ⬜ **[audit2·high] Triathlon taper is distance-blind and hard-capped at 2 weeks — a 70.3/IM athlete 3wk out can get a PEAK week** (Triathlete · A-race) — periodizationPhase enters taper only at weeksToRace 0-2 and takes no distance input, so an Ironman athlete exactly 3 weeks out falls through to the block cycle and can land 'peak/overload — the hardest week of the block' 3 weeks before the race — the opposite of a taper the engine markdown itself prescribes. Route: server/periodization.js:50.
753. ⬜ **[audit2·high] Volume individualization is inoperative without a CTL — the core anchor returns null** (Beginner · no benchmarks) — weeklyLoadBudget returns null when ctl<=0; a no-history athlete has no CTL, so the prompt emits 'CTL×7/×9-11' referencing a number that doesn't exist and weekly volume becomes an ungrounded LLM guess. The mechanism week-shape claims for individualization yields nothing for this persona. Route: server/server.js:2153-2156 (readiness.js:114).
754. ⬜ **[audit2·high] Periodization runs a peak/overload week on a flat health goal — contradicts the load-budget block** (Masters cyclist · health) — The meso-cycle is gated on loadBand!=='maintenance', but 'health' resolves to 'flat', so periodization runs and 1 week in 4 injects a 'PEAK/overload — hardest week, nudge build weeks UP' directive immediately contradicted by the next block ('this is a FLAT week, do NOT chase Form'). Two opposite directives in one prompt; gate should be ==='build'. Masters age-easing is also a no-op off the build band. Route: server/server.js:2131 (week-shape.js:99).
755. ⬜ **[audit2·high] Ride zone analysis + thumbnail hardcoded to FTP 260 — real user FTP ignored** (Completed ride view (any non-JM cyclist)) — ActivityDetail seeds ftp=260 and only overrides from a device-local setting that is NEVER written anywhere in src, and never reads user.ftp/sportSettings. Every non-260 cyclist gets wrong time-in-zone buckets, wrong colors and a literal 'FTP 260 W' label. Plan surfaces seed the same 260. Works for JM, wrong for everyone. Route: src/pages/ActivityDetail.tsx:246.
756. ⬜ **[audit2·medium] Training-days HARD CAP + maxPerDay clamped client-side only; server blind-spreads any number** (Profile + Settings) — PUT /auth/profile does req.user.info={...info,...req.body} with no server clamp; a direct API call can store trainingDays=999/maxPerDay=99. That unvalidated value flows straight into buildSystemPrompt and the 409 guard, defeating the 'hard weekly ceiling' at its source — a safety-adjacent limit enforced only at the UI input. Route: server/server.js:457.
757. ⬜ **[audit2·medium] Mandated theme tokens --amber/--cyan/--warn don't exist → 20+ raw off-palette hex app-wide, incl. semantically-backwards blue for 'low readiness'** (Check-in/readiness, Plan, Stats, Chat, Profile, Admin (all personas)) — styles.css :root defines only --accent/--danger/--text/--line, so the palette the 'always apply tokens' rule relies on is phantom; every warn/amber/cyan usage falls back to invented hex, the 'run-down' verdict paints periwinkle blue (worse-state reads calm), and gym shows two unrelated hues. Convention-only, nothing in the build fails on a raw hex — the exact recurring regression the memory flags. Route: src/styles.css:1-25 (419-426, RunningStats.tsx:25).
758. 🔨 **Every day in the plan horizon must be EITHER a training session OR an explicit REST day — never a silent empty day (JM 2026-07-22, now that rest days exist #735).** The daily-adapt/horizon-fill should mark the non-training days as rest (set_rest_day) so the whole horizon is accounted for (train or rest), closing the "unexplained empty day" gap fully. Route: runDailyAdapt/horizonFill + the coach prompt + set_rest_day. Ties #734/#735.
759. 🔨 **REST DAY as a first-class block in the "+ Add" sheet + rendered as an activity block (JM 2026-07-23).** Today rest lives only as `user.restDates` + a banner (#740) + the coach 409 guard — the USER can't add a rest day from + Add (AddSheet types = ride/run/swim/gym/note; no rest). Add a "Rest day" option to the AddSheet that writes a rest via `set_rest_day`/`restDates`, and render it in the Day/Week/Month view as a real block (same footprint as a workout entry, cyan/rest hue, "Rest & recover" copy + why). So a user aiming for e.g. 6 sessions/wk can drop the rest day themselves, and the coach places it too (#758). Rest = deliberate programmed recovery, not an empty day. Route: src/pages/AddSheet.tsx (add type) + the day-render + calApi/set_rest_day. Ties #740/#758.
760. ⬜ **Capture EVENT / RACE DATES (structured) — in + Add and/or Profile — to DRIVE the periodization phases (JM 2026-07-23).** Today `info.raceDate`/`info.raceName` exist but the coach mostly PARSES the A-race date from free-text goal notes (server.js:2059 "Read the race DATE + DISTANCE from it") — fragile, single-race, no A/B/C priority. For triathletes / multi-sport especially, let the user add important dates (name + date + distance/type + A/B/C priority) as structured EVENTS; surface them on the calendar as event blocks; feed the nearest A-race's `weeksToRace` into `periodizationPhase` (already the anchor: Prep→Base→Build→Peak→Taper→Race, taper is distance-aware #752). Dates DO drive phases — this makes it structured instead of prose-parsed. Route: a new events store + AddSheet/Profile UI + periodization wiring (weeksToRace/taperWeeks). Ties #752/#756.
761. ⬜ **[audit-sim·high] The coach generates NO plan for a brand-new user with no intervals connection + no history.** The whole-app SIMULATION (2026-07-23) triggered a real daily-adapt for 9 fresh personas (teen/masters/swimmer/tri/beginner/female/postpartum/concurrent/bodybuilder) — ALL got 0 plans, while the 2 intervals-connected accounts (jmfiset, xenia) generated full weeks. The plan-create path WORKS for a no-intervals user (verified: a direct POST /api/plan saved fine + the #6 no-benchmark RPE cue fired), so the coach CHOOSES not to build from the profile+shape alone when there's no intervals data/CTL/history. That's an onboarding gap — a new user's first daily-adapt should still lay down a starter week from their profile (sport/goal/trainingDays → weekShape), not leave them empty. Route: runDailyAdapt / dailyAdaptMsg — make horizon-fill build from the profile when there's no intervals history. (Also enables full-persona CONTENT grading in future audits.)
762. ✅→ **[audit-sim·FIXED] Privacy: stale PAST plans retained pregnancy terms.** The sim found Xenia's 07-16/07-21 gym plans reading "pregnancy-safe" — the privacy sweep was future-only + scrubPrivate left a "-safe" artifact. Fixed (ad3d632): scrub ALL plans (all dates), clean hyphenated compounds. Tested.
763. 🧪 **[audit-sim·high] A GYM-focused persona's daily-adapt HANGS (no completion, 0 plans).** The whole-app sim: sim-bodybuilder (gym main sport, hypertrophy) triggered a daily-adapt that logged only the "[chat] req" line — no completion, no tool calls, 0 plans after 5+ min. ROOT CAUSE (transcript): the LLM can't reliably build a full gym week in one pass — it loads `set_gym_target` instead of `create_workout` and drowns in exercise-name search loops. FIX (JM-approved 2026-07-23, "dedicated gym build flow seems sturdy and best"): CODE composes + saves the gym week deterministically (`buildGymWeek` → the #718 assembler, goal-aware) then a LIGHT coach pass adds the voice. **VERIFIED on QA:** bodybuilder builds 11 plans in ~30s (vs 0), strength builds 10; goal-differentiation holds (bodybuilder mains 4×12 hypertrophy, strength 3×5 heavy — JM: "pure strength, not just the bigger dude"). Follow-up polish shipped: `goodExerciseMatch` rejects weak fuzzy exId hits (no leg-curl in an upper warmup) + focus-aware titles (a hypertrophy day reads "Hypertrophy" not "Strength"). Unit-tested (`gym-split.test.ts`). Needs JM QA walk-through + promote.
764. ⬜ **[JM 2026-07-23] Profile "The basics" card padding — content glued to the border.** `.card` has no base padding; the new `.pbasics` section relied on it → the "The basics" title sat flush against the card edge (ugly). Fixed: `.pbasics { padding:14px 16px }` + a hard padding rule added to the options-first UX skill (any new .card MUST add its own padding).

765. ⬜ **[JM 2026-07-23·CRITICAL·bug] A chat reschedule SAVES in Platyplus, is reported "Done", then gets SILENTLY REVERTED by intervals sync.** JM asked the coach (chat, prod, 14:48) to move today's (07-23) Threshold to tomorrow + do an easy ride today ("Can today workout be [moved] tomorrow since I feel a bit sore? Would like a classic ride for today"). TRANSCRIPT (`1a84d0ca…`) proves the coach EXECUTED it correctly: `create_ride {id:mcp-q89528hq, 07-23, "Steady Z2 Endurance"}` (reshaped today→easy, 200 OK) + `create_ride {07-24, "Threshold 4×10"}` (new id mcp-g1h164u0, 200 OK). The coach replied "Done" and printed a "straight from your calendar" confirmation showing 07-23 Steady Z2 / 07-24 Threshold. **But the current prod state is the PRE-SWAP plan:** 07-23 = Threshold 4×10 (a THIRD id `mcp-r2jlyb4m`), 07-24 = EMPTY, no Steady Z2 anywhere. ROOT CAUSE (verified against intervals): the coach's Platyplus writes NEVER reached intervals — intervals event `124775221` "Threshold 4×10" is STILL on 07-23 and there is NO easy-ride/07-24 event. So `reconcileFromIcu` re-imported the stale intervals threshold (124775221, 07-23) as a fresh plan (mcp-r2jlyb4m) → reverted 07-23 back to Threshold AND discarded the coach's Steady Z2 (mcp-q89528hq) + new 07-24 Threshold (mcp-g1h164u0). This is the #588 ownership boundary failing on the coach's OWN reschedule: a `create_ride` that reshapes an intervals-LINKED session (or adds a session on a new day) doesn't move/create the corresponding intervals event, so the next sync makes intervals win and undoes the coach's change. Also the orphaned easy ride JM then did (`i168584107`) had no easy-ride plan to link to (feeds #766). FIXES to investigate: (1) why `pushPlanToIcu` didn't fire/land for these coach writes (prod, non-staging — it should have moved event 124775221 + created a 07-24 event); (2) a proper "move session" MCP op that PRESERVES `icuEventId` and re-pushes, instead of the coach doing reshape+create which orphans the intervals event; (3) reconcile must treat a just-written coach plan change as authoritative over the stale intervals event (don't re-import a superseded event). CRITICAL because the coach tells the athlete "done" while the calendar silently snaps back — maximum trust damage. **ROOT CAUSE CONFIRMED (transcript `1a84d0ca`): the coach's 14:48 chat swap SUCCEEDED (create_ride Steady Z2 on 07-23 + Threshold on 07-24, both 200 + pushed to intervals). Then at 15:06 a "Daily auto-adaptation — EARLY pass" (`db5932c8`) ran and REVERTED it: `remove_workout` the Z2, `set_rest_day 07-24`, `create_ride 07-23 Threshold`. A rescheduled SESSION had NO stickiness — only rest days did (set_rest_day) — so the adapt freely re-planned over the athlete's manual move. 🔨 FIX BUILT (JM "765 yes"): `plan.pinned` flag; the coach sets `pinned:true` on any session it creates/moves per an EXPLICIT athlete reschedule (new MCP param on create_ride/run/workout, exposed in list_schedule). CODE-ENFORCED at every revert path a pinned session can't be: MOVED (upsertPlan coach-move guard → 409), REMOVED (deletePlanById refuses on the coach path → remove_workout 409), or REST-DAYED over (setRestDay → 409). Athlete's own UI actions (actor 'you') still win; the coach releases the pin (pinned:false) first for an explicit re-request. dailyAdaptMsg + tool descriptions instruct respecting/setting pins. 🧪 QA integration test PASSED (scripts/pin-enforcement-test.py): create pinned ride (201, pinned=true) → coach MOVE 409 · coach REMOVE 409 · coach REST-DAY 409 · release pin (200) → move 200. Exactly blocks the adapt-overwrite. VERIFIED END-TO-END on prod: fired the review server-side via the new POST /api/coach/review-activity → coach saved the review (verdict "Easy Z2 recovery — felt strong, fresh legs, well-executed", score 8) + set title "Easy Z2 Endurance" + description + coach_tick=4. The 1st retry (21:35) failed because the coach could not load its MCP tools MID-DEPLOY (transient); a fresh fire after the deploy settled worked. NOTE minor follow-up: postCoachNote's coach_tick PUT occasionally does not stick (had to set it directly, 200) — worth a look but not blocking. IMMEDIATE: JM's 07-23/07-24 calendar was RESTORED via the prod API (Z2 today `mcp-z2jul23r` + Threshold moved to 07-24 `mcp-r2jlyb4m`, both synced to intervals).

766. ⬜ **[JM 2026-07-23·HIGH·bug] An activity rated on Garmin/intervals (feel synced, no Platyplus feedback record) can NEVER get a coach review — app shows "Retry" forever.** VERIFIED end-to-end in prod for JM's easy ride `i168584107`: it has `feel=1` on intervals (rated on the Garmin/Strava side) but there is NO `activityFeedback` entry in the Platyplus store and it isn't linked to a plan. The app marks it "feedback given + Retry" because `ActivityFeedback.tsx:46` trusts the intervals feel (`setSaved(!!(icuExisting?.feel||rpe))`) — but the coach-review gate `hasSessionFeedback` (#723) ONLY counts the Platyplus store → returns false → the daily pass flags `awaitingFeedback` + `/api/coach-review` 409s, and the Retry endpoint (`/auth/activity/:id/review-retry`) finds no store entry / no linked plan → 404, does nothing. So the two "is there feedback?" sources DISAGREE and the review can never fire. Note: JM's rides 07-16→07-21 WERE rated in Platyplus (store entries exist) → they got reviews; that's why only this Garmin-rated one is stuck. FIX options: (A·rec) mirror intervals-side feel/rpe into `activityFeedback` on import/reconcile (or on activity-detail load) so hasSessionFeedback/retry/daily-pass all just work — a Garmin-rated ride gets reviewed automatically; (B) make the review gate + retry ALSO accept a set intervals feel/rpe as feedback (fetch it); (C) retry-only fallback: pull intervals feel when the store is empty and fire; (D) make the app's "saved" state trust ONLY the Platyplus store → shows the entry form so JM logs it in-app (but forces re-entry of feedback already given on Garmin). **JM PICKED A ("best of the best"). 🔨 BUILT (server): `backfillIcuFeedback(user, activity)` mirrors an intervals feel/RPE into `activityFeedback` (feel 1-based → ICU_FEEL_LABELS label; mirrors onto a linked plan id; no-ops if a store record already exists). Hooked in TWO places: (1) `get_recent_activities` backfills every past unreviewed activity in the read window → the daily review pass now catches a Garmin-rated ride once it's ≥2 days old (a fresh today/yesterday one still waits on Retry or in-app submit, by design #681 anti-double-review); (2) the Retry endpoint now fetches the intervals activity + backfills when the store is empty → tapping "Retry" on a Garmin-rated ride produces a review. 🧪 TEST: on prod, JM taps Retry on `i168584107` (feel=1 on intervals, no store record) → a coach review lands + coach_tick set; verify `activityFeedback[i168584107]` gets a `fromIcu:true` record. Immediate unblock without waiting for deploy: open the ride → tap **Edit** → re-save feel+RPE → fires the review.**

767. ⬜ **[JM 2026-07-23·later] Linked planned-workout: show a TITLE + define the planned-vs-completed review + "wrong link" flow.** JM (parked, "to be done later"): (a) add a **title at the top of the linked planned workout** on the completed-activity view so it's clear WHAT the planned session was; (b) confirm the coach reviews **planned vs completed** using that link (compare what was prescribed to what was done); (c) design the flow + recommendation for when the athlete **linked the wrong planned session by mistake** — how to change/unlink the planned↔completed pairing (`activityLinks`), and what the coach should do on re-link (re-review against the corrected plan?). **PARTIAL FIX SHIPPED (#766b, JM 2026-07-23): the review now compares planned-vs-completed against the CORRECT session.** Was: JM's easy Z2 (his actual today plan after the swap) got reviewed against a "Threshold 4×10" that is really the NEXT day's session → the coach flagged a false "miss" and an inconsistent 8/10. Root: the completed activity wasn't linked to today's plan, so the coach guessed the wrong planned session. Fix: `backfillIcuFeedback` LINKS the activity to the same-day same-sport plan; `triggerActivityReview` names that exact planned session in the prompt (+ a fallback to the day's plan, + an explicit "do NOT reference any other day / invent a plan"). 🔨 BUILT (#767 Option A, JM-approved 2026-07-24): a **Planned → Completed** header card at the top of the activity (prescribed title + targets vs what you actually did + a computed verdict — nailed/harder/easier), and **Change** opens the plan picker; on picking a different plan `setLink` re-links AND re-fires the coach review (retryReview → #766b reads the corrected plan) so the review matches. New `PlannedVsCompleted` component + `.pvc-*` styles; ride/run/swim (gym keeps its recap). Typecheck + build green; needs QA look. STILL TODO for #767: (none — both parts shipped; verify on QA) (how to re-pair `activityLinks` + re-review). Route: the activity-detail linked-plan header (src/pages/ActivityDetail.tsx), `activityLinks` (server), the review trigger. Mock the header + the change-link flow (options-first) before building.

768. 🧪 **[JM 2026-07-23·feature] Post-workout GRAPHS: the insight comments say almost nothing — improve "by a mile".** The per-chart insight lines (`ActivityDetail.tsx:116-122` ride, 206 run, 249 swim) are formulaic RESTATEMENTS of the on-screen stat ("NP 152 W · VI 1.05 — steady, even effort"; "Avg 144 bpm — effort held moderate"). They add no value: they don't use the athlete's OWN benchmarks (was this hard/easy for THEM?), their HISTORY/trend (better/worse than usual?), the PLAN (did it hit the intended stimulus?), or any real analytics (aerobic DECOUPLING Pw:HR, EFFICIENCY FACTOR trend, pacing FADE/split, TIME-IN-ZONE quality, W′/anaerobic use). Goal: make each graph surface a genuinely value-adding, non-obvious insight tied to the athlete's numbers + training context — the depth a WKO5/power user pays for, in plain language. Prefer a COMPUTED insights engine (coach-token-thrift — decoupling/EF/splits/zone-quality/benchmark% are pure math) + the coach review for the narrative. Mock 2-3 options first (options-first). Ties: skills platyplus-charts + insights-everywhere; memory platyplus-activity-view; #767 (planned-vs-completed) + beyond-FTP metrics (#403) feed it.

   🔨 BUILT (JM chose "A only" — rework the UNDER-CHART comments, applied to ALL activity types): a computed insight engine `src/activity-insights.ts` (pure, unit-tested `src/activity-insights.test.ts`, 13 tests) turns each chart's weak restatement into a real read — aerobic DECOUPLING (Pw:HR / Pace:HR drift = durability), EFFICIENCY FACTOR + its trend vs the athlete's recent weeks ("efficiency up 4%"), TIME-IN-ZONE quality, PACING split (fade vs even/negative), and BENCHMARK % (hard/easy for THEM). Wired into RideTimeline/RunTimeline/SwimTimeline (rich 2-line `InsightRead`) + the GYM read in GymSummary (names the PR lift + progression + muscle coverage, NO tonnage per #601). efBaseline computed from the last ~35 days of same-sport activities. Degrades gracefully with no benchmark/HR. 🧪 TEST: on QA, open a completed ride/run/swim/gym → each chart's comment now says something non-obvious (decoupling %, EF trend, zone %, pacing) instead of "avg 144 bpm, moderate".

769. ⬜ **[JM 2026-07-24·directive] Audit simulations MUST exercise real USER INTERACTIONS, not just plan generation — "test like crazy".** The whole class of #765-type bugs (a chat reschedule silently reverted) only shows up when you simulate what a user actually DOES to their plan via the coach. The sim/audit battery must include, per persona: ask the coach to **MOVE a workout** to another day (and verify it STAYS after the next adapt), **DELETE/skip one**, **SUBSTITUTE one** (e.g. "do an easy ride instead of the threshold today"), try to **set a REST DAY on a day that already has a workout** (expect the swap/clear flow, no silent stack), try to **add MULTIPLE workouts to one day** (expect the max-per-day 409 / combine), exceed the **weekly training-days cap**, place a **heavy gym next to a quality endurance day** (concurrent-interference 409), and re-request a change (pin release). VERIFY the END STATE (DB + intervals) matches the request AND survives the next daily-adapt — the coach saying "done" is NOT enough (#765). Update the coach-simulation + whole-app-audit skills with this interaction matrix; build a repeatable interaction-test battery (scripts/) that hits the guards (planCapViolation, trainingDays cap, rest-day guard, concurrent-interference, #765 pin). "test like crazy for all personas" (JM).

### AUDIT RECONCILIATION (2026-07-24, live QA sim + code verification)
Ran live daily-adapt across 12 personas + verified the engine gaps in code. FINDINGS:
- **The engine gaps #752–757 are ALL FIXED** (this session/prior — log was just stale): #752 distance-aware taper (70.3→3wk, IM→4wk, `periodization.js:68`), #753 no-CTL ramp-in prompt (`server.js:2312`), #754 flat/maintenance→'steady' NO peak week (`periodization.js:78-83`), #755 real account/activity FTP (no 260 hardcode, `ActivityDetail.tsx:373-380`), #756 server-side cap clamp (`server.js:460-463`), #757 theme tokens `--amber/--cyan/--warn` exist in `:root` + `check-theme.mjs` guard. → mark resolved.
- **#761 was a MISDIAGNOSIS**: the 8 no-icu personas got 0 plans because they were MISSING `sex` (+ eEkwi missing mainSport transiently) → the profile-gate (#A) CORRECTLY skips an incomplete profile. Not a coach refusal. With complete profiles, they generate full weeks. → the app is behaving right; close #761 as not-a-bug (the gate is the intended behaviour).
- **Generation WORKS across all sports for complete profiles**: tri (all 3 sports balanced), female cyclist (VO2/threshold/cadence variety + strength), teen (easy + strides ONLY, no maximal — SAFE), swimmer (CSS sets), running, cycling, gym (goal-differentiated). Content quality is high + individualized + safety-correct.
- **Interaction guards GREEN** (#769, 9/9): move/delete/substitute/rest-conflict/multi-workout/concurrent/training-days all code-enforced.
- Remaining genuine work = FEATURE completeness (#758 every-day-train-or-rest, #759 rest as a first-class block, #760 structured event dates), not correctness bugs. Sim caveat: firing 12 coaches at once saturates the helper → some show 0 (timing), not failures.

770. ⬜ **[JM 2026-07-24·bug] Day view shows "today"-framed check-in copy when viewing a NON-today day.** Viewing Thu Jul 23 (`?d=2026-07-23`) while today is Jul 24, the check-in card still reads "✓ CHECKED IN … Your coach has today's check-in" and "You're fresh — good to train as planned" — today-anchored language on a past/other day. The check-in + readiness banner should only use "today" phrasing (and arguably only show at all, or show as a historical check-in) when the viewed day === today; otherwise it misleads (a past check-in reads as if it's current). Route: the Plan Day view check-in card + readiness banner — gate the "today" copy / card on viewedDate === localToday.

771. ⬜ **[JM 2026-07-24·HIGH·bug (prod)] The plan's quality sessions DON'T follow the athlete's development priority — coach states VO₂-ceiling but prescribes threshold.** JM's profile card computes "Diesel engine — raise the ceiling" (TTE 57min already long → the gain is VO₂/short-max, NOT more threshold duration). But his prod plan is threshold + sweet-spot HEAVY: Threshold 4×10 @98%, Threshold 3×15 @98% (MORE threshold duration — the exact thing his profile says he doesn't need), lots of Sweet-Spot @90%, and only ONE weak "ceiling effort" @105% — zero real VO₂ (108-112%). ROOT: `assignQuality` (server/archetypes.js:51) rotates quality archetypes for VARIETY (+ skip-recent + ceiling) but is PRIORITY-BLIND — it doesn't bias toward the athlete's # DEVELOPMENT PRIORITIES (VO₂ for a ceiling-raiser, threshold/TTE for a diesel who needs duration, base/EF for a low-EF athlete). So the priority is prompt-only (LLM ignores it, defaults to the safe threshold/sweet-spot). This is the audit red-flag "metric shown but not prescribed-from." FIX: make `assignQuality`/`assignArchetypeBlock` WEIGHT the archetype pool toward the development priority (a ceiling-raiser gets VO₂-led quality — ~2 VO₂/wk as the key sessions, threshold/SS as support), so the CODE that picks sessions reflects the athlete's own numbers. Ties #620 (archetypes) + #403 (beyond-FTP metrics) + validate-athlete-types (metrics must DRIVE prescriptions).
