...
| **Change intervals↔Platyplus sync** (`reconcileFromIcu`, `icuEventToPlan`, `planToIcuEvent`) | **Platyplus-WINS for CONTENT; intervals-move-wins for the DAY** (#380). GYM has no intervals model → never re-import our own gym events (they carry the "Open in Platyplus" link → skip, else empty shell, #377). The auto deep-link must never persist in `plan.notes` → `stripPlatyplusLinks()` on every compose/import (#378). ⚠️ QA + prod share the real athlete i28814 — cross-env pushes/reconciles collide. |
