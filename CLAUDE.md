...
| **Change intervals↔Platyplus sync** (`reconcileFromIcu`, `icuEventToPlan`, `planToIcuEvent`) | **Platyplus-WINS for CONTENT; intervals-move-wins for the DAY** (#380). GYM has no intervals model → never re-import our own gym events (they carry the "Open in Platyplus" link → skip, else empty shell, #377). The auto deep-link must never persist in `plan.notes` → `stripPlatyplusLinks()` on every compose/import (#378). ⚠️ QA + prod share the real athlete i28814 → **`IS_STAGING` makes QA READ-ONLY toward intervals** (`pushPlanToIcu` skips + `dailyAdaptTick` returns on staging; only PROD writes). Never let prod compute `IS_STAGING=true` (it'd stop mirroring) — it's derived from the QA `RP_ID`/`ORIGIN`. #381. Push finds a MOVED event by `icuEventId` (any date) so it updates, not duplicates. |
