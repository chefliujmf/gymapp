...
| **Per-user intervals athlete (never leak i28814)** | the client reads its intervals athlete from a device-local `icu_athlete_id` that DEFAULTS to the seed athlete i28814 → a shared/unsynced browser leaks JM's data to another user (#453, Xenia saw JM's activities). The **server `/icu` proxy FORCES `/athlete/<id>` to the authenticated `req.user.icuAthlete`** (authoritative); the client `syncIcu` (AuthContext) ALWAYS writes the current user's athlete. Don't reintroduce an unconditional i28814 default on any per-user read. |
