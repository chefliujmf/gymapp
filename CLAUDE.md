...
3. **Every fix ships with a test.** A unit test in `src/*.test.ts` (`npm test`, vitest) when the logic is
   pure; otherwise a `scripts/smoke-test.mjs` row and/or a **manual step in `REGRESSION.md`**. The test is
   the permanent regression net — write it, run it green, commit it.
4. **`REGRESSION.md`** is the live "test one-by-one" guide: per item = unit test file + JM's manual steps +
   expected result + status. Keep it current.
