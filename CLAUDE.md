...
| **Change how the coach writes an activity's PUBLIC title/description** (`set_activity_text` MCP tool = the source of truth for the wording) | Keep BOTH title AND description **PLAIN** (JM 2026-07-16, supersedes the old "human title / scientific description"): TITLE = the session's type/zone/purpose ("Easy Z2 Endurance", "Sweet-Spot 3×12"); DESCRIPTION = one short plain sentence naming the zone/type + what it's for. NO location, NO repeated numbers, NO physiology lecture, NO em-dash, never mention pregnancy. Edit the rules in the `set_activity_text` tool description (mcp/server.js). Memory `platyplus-coach-public-text-voice`. (#425) |
| **Edit the coach engine** (`server/coach-engine*.md`) | These are now **HAND-MAINTAINED — edit the shipped `server/coach-engine-*.md` files DIRECTLY.** ⚠️ `scripts/sync-coach-engine.mjs` + `coach-engine-src/` are **DEPRECATED and UNSAFE to run (#603)**: the shipped engines drifted (direct edits #479/#614 not back-ported; deactivated sections trimmed) so a re-sync silently REVERTS work — the script now refuses unless `COACH_ENGINE_SYNC_FORCE=1`. Reconciling the drift (back-port to source, or retire the source) is the open #603. Baked into the server image via `COPY`; reaches prod on the next promote. |
