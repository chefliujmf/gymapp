...
// The HEADER ships to the coach ×3 engines every turn, so keep it TIGHT (token-thrift #590). A maintainer note
// lives only as an HTML comment (dev-facing); the prose is coach-facing only.
const HEADER = (what, note) => `<!-- GENERATED by scripts/sync-coach-engine.mjs from coach-engine-src/ — edit the SOURCE, not this file, then re-run the sync. -->
# Platyplus coaching engine — ${what}
...
- PUBLIC (syncs to Strava): activity TITLE + DESCRIPTION via \`set_activity_text\` — public-safe ONLY, plain, no
  score/health/feelings/plan, no em-dash.
