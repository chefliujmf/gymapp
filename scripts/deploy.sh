...
#   • On the XPS itself (self-hosted Actions runner): set DEPLOY_LOCAL=1. Builds
#     using the synced content dir (DOWNLOADED_PAGES_DIR) → sync into the compose
#     dir → rebuild/restart → health-check.
#
# Env:
...
wait_healthy() {
  ...
  for i in $(seq 1 30); do
    ...
    [ "$status" = healthy ] && { echo "✓ gymapp healthy"; return 0; }
    sleep 2
  done
  echo "✗ gymapp did NOT become healthy — recent logs:" >&2
  if [ "$runner" = local ]; then ( cd "$APP_DIR" && docker compose logs --tail 40 gymapp ) >&2
  else ssh "$XPS_HOST" "cd $APP_DIR && docker compose logs --tail 40 gymapp" >&2; fi
  return 1
}
